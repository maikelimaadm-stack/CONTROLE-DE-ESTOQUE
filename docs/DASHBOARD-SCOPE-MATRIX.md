...
| /dashboards/feedlot | dashboard.feedlot.view (confinamento) | r | feedlot_corrals → sectors → yards + animals + batches | y.farm_id; a.farm_id; b.farm_id | `{{escopo:y.farm_id}}` no pátio e, na subconsulta de ocupação, `{{escopo:a.farm_id}}` + `{{escopo:b.farm_id}}` — animais e lotes têm empresa própria e a ligação com o curral é chave de negócio (o lote da empresa B num curral do pátio da empresa A era contado) |
