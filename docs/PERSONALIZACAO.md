...
1. **Barra superior**: ícone de filtros (abre o painel lateral "Filtros"), botão verde **Novo** (e **Excluir** quando há um registro selecionado); à direita: pesquisa (ícone → caixa "Pesquisar…", Enter aplica), ocultar/exibir faixa de filtros, alternância **Registro / Tabela / Cards** e **Mais opções** (Duplicar, Imprimir, Exportar Excel/CSV, Histórico, Configurações, Exportar PDF, Relatório personalizado, Salvar tela como padrão da organização, Restaurar padrão da tela).
