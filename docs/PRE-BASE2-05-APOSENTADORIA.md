...
| **05C-2** — o contador | **fatia ATIVA**: `supabase/migrations/0018_empresa_code_sequence.sql` escrita e provada (fresh, upgrade com acervo, fail-closed em 8 estados impossíveis, concorrência com conexões reais, matriz de version skew em `pnpm gate:05c2`). **PR DRAFT.** Migration **NÃO executada** em produção e janela single-version **NÃO realizada** — o gate de quiesce está `BLOCKED`, e o runbook é `docs/PRE-BASE2-05C-2-CUTOVER.md` |
