...
`935f9dc`), o cutover foi executado dentro de janela single-version e validado — ver "Onde a 05C está
agora". **O contador canônico é `entity='empresa'`**, com o `last_value` preservado da chave anterior.
A sequência `entity='farm'` nunca foi assunto da 05C-1 — foi exatamente o assunto da 05C-2, e dizer o
contrário já produziu plano errado uma vez.

As **lápides** que permanecem por decisão própria — `contrato-legado.ts`, os redirects de rota antiga — não
reabrem a PRE-BASE2-05. Elas são compatibilidade de NAVEGAÇÃO e de leitura histórica, com dono e data de
saída próprios; a aposentadoria do idioma antigo no cliente, no servidor e no schema está encerrada.
...
| **05C-2** (concluída, mesclada na PR #37 — `935f9dc` — e aplicada em produção em 16/09/2026: `0018_empresa_code_sequence.sql`) | a troca do contador: renomeia a chave persistida `entity='farm'` → `'empresa'` preservando `last_value`, e vira a constante `SEQUENCIA_EMPRESA` junto | não copia o contador (duas linhas vivas colidem); não toca em `farm_transfer` nem em nenhum outro contador; não executou o cutover por conta própria — a janela single-version é humana, e foi conduzida pelo Maike em 16/09/2026 |
...
| **05C-2** — o contador | **CONCLUÍDA**. `supabase/migrations/0018_empresa_code_sequence.sql` provada (fresh, upgrade com acervo, fail-closed em 9 estados impossíveis, concorrência com conexões reais, matriz de version skew em `pnpm gate:05c2`), mesclada na PR #37 (merge `935f9dc`) e **aplicada em produção em 16/09/2026**, uma única vez, dentro de janela single-version: deployment BASE `a9df04a8…` removido e zero réplicas antes do merge; depois, ledger em `0018_empresa_code_sequence.sql`, `entity='farm'` → 0, `entity='empresa'` → 1, `last_value` = 2 preservado, `empresas.code` `[1,2]`, `GET /health` 200. O gate de quiesce, que esteve `BLOCKED`, foi resolvido na própria janela. Runbook com a execução registrada: `docs/PRE-BASE2-05C-2-CUTOVER.md` § *Encerramento real — 16/09/2026* |
