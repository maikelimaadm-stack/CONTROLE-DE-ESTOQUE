...
Total medido: **2640** ocorrências em 10 superfícies · 50 tabelas com coluna de empresa.

## Por superfície

| Superfície | Ocorrências | Catraca | Observação |
| --- | ---: | --- | --- |
| Schema (migrations) | 153 | sim | não pode crescer |
| API — código | 990 | sim | não pode crescer |
| API — testes | 193 | sim | não pode crescer |
| Pacotes compartilhados | 116 | sim | não pode crescer |
...
| Documentação ativa | 175 | não | documentação: acompanha a migração |
...
| --- | --- | ---: | --- |
| `farm_id` | dado | 1099 | `empresa_id` |
| `farms` | dado | 108 | `erp.empresas` |
| `member_farms` | dado | 33 | `member_empresas` |
| `ctx_farmId` | contrato | 153 | `empresaSelecionada` |
| `farmIds` | contrato | 81 | `empresasPermitidas` |
| `x_farm_id` | contrato | 40 | `X-Empresa-Id` |
| `farmScope` | contrato | 43 | `escopoEmpresa` |
| `allowedFarms` | contrato | 120 | `companyScope (@agro/platform)` |
| `farms` | contrato | 109 | `/empresas` |
| `fazenda` | texto | 854 | `Empresa (i18n: termos.empresa)` |
