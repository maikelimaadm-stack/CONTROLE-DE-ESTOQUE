...
| 5 | **PRE-BASE2-05** — Contexto multiempresa | Seletor "todas / uma / conjunto", filtros e painéis consolidados, seleção obrigatória no lançamento, seletor de idioma. ✅ **concluída em produção** (aposentadoria encerrada pelo cutover do contador em 16/09; ver `PRE-BASE2-05C-2-CUTOVER.md`) | 02, 03 |
| 6 | **BASE2-01** — Moldura de lançamento | Shell oficial do Modelo Base 2 (cabeçalho, dados principais × itens, totais, histórico, ações). Contrato em `MODELO-BASE2-CONTRACT.md`; implementação em `apps/web/src/features/base2/`; piloto no detalhe de documento de estoque. 🟡 **implementação em PR — não implantada** | 05 |
