...
| 4 | **PRE-BASE2-04** — ID Global | Alocação automática nas 29 portas de escrita + porta genérica, backfill operacional determinístico e retomável, resolução na busca global (`#55`, `ID 55`) e exibição do `#N` no registro. 🟡 **implementação pronta — ativação em produção pendente** | 01 (03 recomendada) |
