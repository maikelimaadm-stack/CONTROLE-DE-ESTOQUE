...
| 5 | **PRE-BASE2-05** — Contexto multiempresa | Seletor "todas / uma / conjunto", filtros e painéis consolidados, seleção obrigatória no lançamento, seletor de idioma. 🟡 **cutover 05C-2 executado em produção em 16/09** (merge `935f9dc`; `0018` no ledger uma única vez; `entity='farm'` = 0 e `entity='empresa'` = 1) — **mas o encerramento DOCUMENTAL não foi feito**: `PRE-BASE2-05C-2-CUTOVER.md`, `DEPLOYMENT.md`, `PRE-BASE2-05-APOSENTADORIA.md` e um teste verde ainda dizem `BLOCKED`. Enquanto os cinco não forem atualizados numa fatia própria, esta fase **não** conta como encerrada para efeito de "cada fase só começa com a anterior em produção e comprovada" | 02, 03 |
| 6 | **BASE2-01** — Moldura de lançamento | Shell oficial do Modelo Base 2 (cabeçalho, dados principais × itens, totais, histórico, **anexos**, ações). Contrato em `MODELO-BASE2-CONTRACT.md`; implementação em `apps/web/src/features/base2/`; piloto no detalhe de documento de estoque. 🟡 **implementação em PR — não implantada.** ANEXOS: **NÃO FEITO** nesta fatia porque o servidor recusa as entidades de estoque com 422 (`ATTACHMENT_PARENTS`); a decisão de manter ou retirar o item do escopo é do Maike | 05 |
