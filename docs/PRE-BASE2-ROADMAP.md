...
| 9 | **PORTAIS + TOP CONFIGURÁVEL** — Portais operacionais | Portal de Compras, Portal de Vendas, Movimentações de Estoque e Movimentações Financeiras como experiência única de lançamento, com TOP cadastrada previamente pelo cliente. Contrato em `docs/PORTAIS-OPERACIONAIS-CONTRACT.md`. **TOP-CONFIG-01** (cadastro versionado de Tipos de Operação: `erp.tipos_operacao`, API administrativa, tela em Configurações › Operações, permissões, RLS, auditoria): ✅ **MESCLADA** (PR #46). **TOP-CONFIG-02** (primeiro lançamento escolhendo uma TOP cadastrada; piloto Portal de Vendas / `erp.sales_documents`; snapshot `tipo_operacao_id` + `tipo_operacao_versao_id` na 0021, endpoint operacional por variante, conversão com TOP do destino, descoberta de capacidade no cliente): 🟡 **EM PR**. O documento NÃO passou a decidir efeito: confirmar venda continua fazendo o que fazia. A camada configurável NÃO substitui o registry de famílias canônicas da BASE2-02 — são duas camadas, e `docs/TIPO-OPERACAO-CONTRACT.md` §0 declara qual é qual. | 07 |
| 10 | **DATA-GOV** — Governança de dados | Nomenclatura final, dicionário com cobertura certificada, rótulos de enum por i18n. | 03, 08 |
...
| **BASE2-03C** — Vendas / Documento de Venda | `erp.sales_documents` · `/vendas/budgets/:id`, `/vendas/orders/:id`, `/vendas/sales/:id` | ✅ **CLOSED / IMPLANTADA EM PRODUÇÃO / CERTIFICADA** | TRÊS variantes da MESMA entidade (`kind` = `budget` / `order` / `sale`), uma fatia só, três famílias de capacidade (`budgets.*`, `orders.*`, `sales.*`). Fechou a fronteira de variante (carregamento amarrado ao `kind`, rota errada = 404) e a autorização COMPOSTA da conversão (`source.edit` ∧ `target.create`), e migrou o detalhe para o Modelo Base 2. Zero migration. Merge `120c117ad760dd953a3dcac1a065967bd6ef2f78` (PR #44), HEAD aprovado `6ec4cd2ce6eb75ac431619d8af2615dd9e5e0fd1`, com `tree` do merge idêntica à do HEAD aprovado (`8e79471c…`) e `diff` vazio entre os dois — nada entrou em `main` que não estivesse revisado · CI do merge **4/4 SUCCESS** (run `35449747673`, `event=push`, `head_sha` igual ao merge; único run nesse SHA) · Railway API `1cbbd208-060f-4fe9-92e8-49ec81dd8838` e WEB `2b7e765f-455d-4b19-ba24-a9483efe2ab2`, ambos SUCCESS no ambiente `production`, ambos `latestDeployment`, com `commitHash` exatamente igual ao merge · Vercel `success` **pelo commit status do GitHub** — a API da Vercel NÃO foi consultada (403 de escopo), e essa é a única fonte usada · `/health` 200 com `db: ok`, com `uptime` ancorado no boot desse deployment · boot registrou `migrations aplicadas: []` · ledger de produto medido em 19 migrations, `count(distinct name)` 19, topo `0019_warehouse_transfer_code_sequence.sql`, `0019` 1×, zero `0020+`. **PENDING declarado:** o smoke autenticado em produção (abrir orçamento, pedido e venda reais e conferir identidade por `d.kind`, TOP, empresa, itens e total) NÃO foi executado — exige sessão que a automação não tem, e CI/E2E rodam em banco efêmero, que é ambiente diferente. |
...
| PORTAIS + TOP CONFIGURÁVEL | Não cria motor genérico: efeito configurável (estoque, financeiro, fiscal, contábil), workflow, campos obrigatórios dinâmicos, layout dinâmico, expressão, SQL configurável e webhook ficam fora até que uma fatia os autorize com contrato próprio. Não materializa as famílias canônicas como linhas no banco (seriam duas fontes de verdade). Não persiste TOP em documento — isso é TOP-CONFIG-02. |
