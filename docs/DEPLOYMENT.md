...
## Vercel (web)
- Projeto Git com **Root Directory = `apps/web`** (a Vercel instala o workspace pnpm a partir da raiz); `apps/web/vercel.json` compila `@agro/shared` e `@agro/domain` antes do `next build`. `output: "standalone"` fica desativado na Vercel (variável `VERCEL`), pois quebra o rastreamento de arquivos da plataforma.
...
| Vercel (frontend) | **Em produção**: projeto `controle-de-estoque` (Root Directory `apps/web`, Next.js, Node 22) com domínios `https://controle-de-estoque-erp.vercel.app` e `https://controle-de-estoque-api-eight.vercel.app`; proteção SSO da Vercel desativada (o sistema tem login próprio); variáveis `NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`. `WEB_ORIGIN` da API na Railway aponta para esses dois domínios. Verificado com navegador real: login do owner → `/` com painel carregado (chamadas `auth/login`, `auth/me`, `auth/context`, `admin/notifications`, `dashboards/home` todas 200). | deploy de produção `2cf776f` READY; screenshot em sessão |
