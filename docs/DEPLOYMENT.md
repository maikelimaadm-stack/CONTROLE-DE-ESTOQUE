...
|---|---|---|---|---|---|---|
| **G1** | PR #56 implantada; `0023` no ledger com o gatilho da R1; `TOP_EFFECTS_RUNTIME_V1_ENABLED` ausente ou `0` | Maike (gate); sessão (leitura) | Supabase (leitura); Railway, serviço `api` → Variables | blocos 5 e 6 do inventário; nome da variável no painel (o valor não precisa ser lido por sessão) | ledger e gatilho **OK em 23/09/2026** (inventário acima); gate da TOP **PENDING** (conferência no painel) | sim |
...
| **G6** | credenciais conhecidas e variáveis de seed neutralizadas: (a) usuários `@demo.local` desativados ou com senha trocada; (b) `LOCAL_AUTH_SECRET` forte (≥ 32 caracteres aleatórios, nunca o padrão de desenvolvimento); (c) `NEXT_PUBLIC_DEMO_MODE` ausente em cada serviço web listado em "## Superfície web"; (d) `SEED_ON_DEPLOY=0` ou ausente | Maike | (a) Configurações → Usuários, na organização sandbox; (b)(d) Railway `api` → Variables; (c) Railway `web` e Vercel → Environment Variables | (a) bloco 2 do inventário (`operador@demo.local` com `is_active = false`); (b) só o Maike confere o valor — nenhuma sessão lê segredo; (c)(d) presença/ausência do NOME da variável | (a) **PENDING** — `operador@demo.local` ativo em 23/09/2026; (b) **PENDING**; (c) Railway `web`: ausente em 23/09/2026 (**OK**), Vercel **PENDING**; (d) **PENDING** — `SEED_ON_DEPLOY` ainda definida no `api` em 23/09/2026 | sim |
...
1. **Escolher o e-mail do dono novo** — um destes caminhos, nunca os dois:
   - (a) usar um e-mail que ainda não existe no sistema; ou
   - (b) manter o e-mail do Maike: ANTES, na `principal` → Configurações → Usuários, trocar o e-mail do
     usuário atual para outro (ex.: um apelido `+sandbox`), e conferir pelo bloco 2 do inventário que o e-mail
     real sumiu da tabela. Se a tela não permitir trocar o e-mail, use (a).
