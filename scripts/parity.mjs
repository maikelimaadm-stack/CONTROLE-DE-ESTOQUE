...
import { LEGACY_REDIRECTS } from "../apps/web/redirects.mjs";
...
/**
 * Reorganização funcional (docs/UX-ARCHITECTURE.md): telas de listagem que passaram a viver como aba/sub-aba de uma
 * área unificada. A equivalência vem da própria tabela de redirecionamentos do web (fonte única), portanto uma tela
 * "UNIFICADA" continua acessível pela rota antiga e pela nova — é melhoria de navegação, não perda de função.
 */
const redirectMatchers = LEGACY_REDIRECTS.map((rd) => { const re = new RegExp("^" + rd.source.replace(/:([a-z]+)\(([^)]*)\)/g, (_m, _n, alts) => `(${alts})`).replace(/:([a-z]+)/g, "([^/]+)") + "$"); return { ...rd, re }; });
function unifiedArea(ours) {
  if (!ours) return "";
  const [pathname, query = ""] = ours.split("?"); const qs = new URLSearchParams(query);
  for (const rd of redirectMatchers) {
    const m = rd.re.exec(pathname); if (!m) continue;
    if (rd.has && !rd.has.every((h) => h.type === "query" && qs.get(h.key) === h.value)) continue;
    let dest = rd.destination; const names = [...rd.source.matchAll(/:([a-z]+)/g)].map((x) => x[1]); names.forEach((n, i) => { dest = dest.replace(`:${n}`, m[i + 1]); });
    return dest;
  }
  return "";
}
const rows = inv.map((r) => { const base = { ...r, ...resolve(r) }; const area = base.kind === "list" || base.kind === "report" ? unifiedArea(base.ours) : ""; if (!area) return { ...base, area: "" }; const status = base.status === "IMPLEMENTADO" || base.status === "MELHORADO" ? "UNIFICADO" : base.status; const note = (base.note ? base.note + "; " : "") + `UNIFICADO: aba da área ${area.split("?")[0]} (rota antiga redireciona)`; return { ...base, area, status, note }; });
const unmapped = rows.filter((r) => r.unmapped);
const ORDER = ["TESTADO", "IMPLEMENTADO", "MELHORADO", "UNIFICADO", "EM IMPLEMENTAÇÃO", "MAPEADO", "BLOQUEADO", "NÃO INICIADO", "NÃO APLICÁVEL"];
...
out("MODULE-PARITY.md", hdr("Paridade de Módulos", `Módulos do menu da referência × cobertura nossa (telas). Total geral: ${pct(rows)}.`) + "| Módulo | Telas ref. | Implementadas/Testadas/Melhoradas/Unificadas | Em implementação/Mapeadas | Não iniciadas | Não aplicáveis | Cobertura | Status do módulo |\n|---|---|---|---|---|---|---|---|\n" + mods.map((m) => { const l = rows.filter((r) => r.module === m); const c = count(l); const ok = c.TESTADO + c.IMPLEMENTADO + c.MELHORADO + c.UNIFICADO; const st = ok === l.length ? "IMPLEMENTADO" : ok > 0 ? "EM IMPLEMENTAÇÃO" : "NÃO INICIADO"; return `| ${m} | ${l.length} | ${ok} | ${c["EM IMPLEMENTAÇÃO"] + c.MAPEADO} | ${c["NÃO INICIADO"] + c.BLOQUEADO} | ${c["NÃO APLICÁVEL"]} | ${pct(l)} | ${st} |`; }).join("\n") + `\n\n## Módulos nossos sem equivalente direto (MELHORADO)\n\n- Ledger de estoque imutável com estorno (\`/estoque/movimentos\`) — a referência edita/exclui movimentos.\n- Auditoria consultável (\`/admin/auditoria\`) e notificações por regra (\`/admin/notificacoes\`).\n- Exportações genéricas de qualquer cadastro (\`/integracoes/exportacoes\`).\n- Painel fiscal com status honesto de cada capacidade (\`/fiscal\`).\n`);
// REPORT
