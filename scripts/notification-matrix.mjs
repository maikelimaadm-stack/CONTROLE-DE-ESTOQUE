...
linhas.push("conta sem o limite da janela, então em caixas com mais de 50 avisos visíveis ele é maior que a lista.");
linhas.push("É diferença de JANELA, não de autoridade: nada contado está fora do que o usuário pode ver. O número");
linhas.push("sai de UMA função (`contarNaoLidas`), servida tanto por `GET /admin/notifications` (campo `unread`)");
linhas.push("quanto por `GET /auth/context` (`unreadNotifications`) — duas redações da mesma regra divergiriam na");
linhas.push("primeira edição de só uma delas. O badge lê o campo da CAIXA, que é pollada; ler o contexto, que não");
linhas.push("é, deixava o badge congelado enquanto a caixa já mostrava o número novo.");
linhas.push("");
linhas.push("A contagem tem um TETO de custo (500, não a janela de 50): ela avalia `erp.tem_acesso_empresa` linha a");
linhas.push("linha e roda a cada 60 s por aba aberta — medido com `EXPLAIN ANALYZE` sobre 200 mil avisos, ~7,6 s");
linhas.push("exata contra ~130 ms com teto. Acima dele a resposta marca `unreadTruncado` e o badge mostra \"500+\".");
linhas.push("");
