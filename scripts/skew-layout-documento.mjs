#!/usr/bin/env node
/**
 * DECIDE, PARA O JOB DE VERSION SKEW, SE A API DA BASE JÁ DECLARA O LAYOUT DO DOCUMENTO (VENDAS-A3-1).
 *
 *   node scripts/skew-layout-documento.mjs              # imprime a decisão e o motivo
 *   node scripts/skew-layout-documento.mjs --github-env # e exporta SKEW_BASE_TEM_LAYOUT_DOCUMENTO
 *
 * Gêmeo de `scripts/skew-condicao-pagamento.mjs`: a base vem de `SKEW_BASE_COMMIT` (declarada pela execução)
 ...
 * ambiente é mutável por fora e só serve de CONFERÊNCIA. A regra inteira mora em `lib/layout-documento.mjs`.
 */
import { appendFileSync, existsSync, readFileSync, writeFileSync } from "node:fs";
import { dirname, join, resolve } from "node:path";
import { fileURLToPath } from "node:url";
import { ARQUIVO_DECISAO, decidir, ocorrenciasNoCommit } from "./lib/layout-documento.mjs";
...
try {
  ...
  console.log(`[skew/layout-documento] base ......... ${sha} — ${origem}`);
  console.log(`[skew/layout-documento] ocorrências .. ${d.ocorrencias} (declaração de \`capacidades.layoutDocumento\` na árvore da base)`);
  console.log(`[skew/layout-documento] decisão ...... a base ${d.declara ? "DECLARA" : "NÃO declara"} o layout do documento`);
  console.log(`[skew/layout-documento] motivo ....... ${d.motivo}`);
  writeFileSync(join(RAIZ, ARQUIVO_DECISAO),
    `${JSON.stringify({ baseSha: sha, ocorrencias: d.ocorrencias, declara: d.declara }, null, 2)}\n`);
  console.log(`[skew/layout-documento] decisão gravada em ${ARQUIVO_DECISAO}`);
  if (process.argv.includes("--github-env") && process.env.GITHUB_ENV) {
    appendFileSync(process.env.GITHUB_ENV, `SKEW_BASE_TEM_LAYOUT_DOCUMENTO=${d.declara ? "1" : "0"}\n`);
    console.log(`[skew/layout-documento] exportado SKEW_BASE_TEM_LAYOUT_DOCUMENTO=${d.declara ? "1" : "0"}`);
  }
} catch (e) {
  console.error(`[skew/layout-documento] REPROVADO: ${e.message}`);
  process.exit(1);
}
