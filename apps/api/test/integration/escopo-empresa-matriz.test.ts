...
/**
 * Matriz de escopo entre EMPRESAS (hardening): Organização O, Empresa A, Empresa B.
 *  - OWNER (sem restrição de escopo) → A + B
 *  - USER_A  (escopo = [A]) → só A; registro de B por id → 404; X-Empresa-Id=B → 403
 *  - USER_AB (escopo = [A,B]) → A + B
...
let A: Hdr; let AB: Hdr; let OWNER: Hdr; let empresaA: string; let empresaB: string; let whA: string; let whB: string;
...
async function member(name: string, email: string, empresaIds: string[]): Promise<Hdr> {
  ...
  const mem = await h.app.inject({ method: "POST", url: "/api/admin/members", headers: h.headers(), payload: { name, email, password: "Matriz@12345", role_id: j(role).id, escopos_empresas: escoposDeTodosOsModulos(empresaIds) } });
...
async function matrix(label: string, base: string, a: string, b: string, listUrl = base, itemsKey = "items") {
  const items = (r: ReturnType<typeof j>) => (r[itemsKey] as { id: string; empresa_id?: string }[]) ?? [];
  // USER_A sem X-Empresa-Id: lista só A; detalhe A 200; detalhe B 404 (não expõe existência)
  ...
  // USER_A + X-Empresa-Id=A → permitido; X-Empresa-Id=B → 403 já no contexto
  expect((await get(`${base}/${a}`, { ...A, "x-empresa-id": empresaA })).statusCode, `${label}: USER_A X-Empresa-Id=A`).toBe(200);
  expect((await get(`${base}/${b}`, { ...A, "x-empresa-id": empresaB })).statusCode, `${label}: USER_A X-Empresa-Id=B`).toBe(403);
  // USER_AB e OWNER → A e B (lista e detalhe); OWNER com X-Empresa-Id=A → só A (seleção continua sendo filtro válido)
  ...
  const sel = items(j(await get(listUrl, { ...OWNER, "x-empresa-id": empresaA }))); expect(sel.some((x) => x.id === b), `${label}: OWNER X-Empresa-Id=A não traz B`).toBe(false);
}

beforeAll(async () => {
  h = await harness(); I = await ids(h); empresaA = I.empresa; empresaB = I.empresa2; whA = I.warehouse!; whB = I.warehouseEmpresa2!; OWNER = h.headers();
  A = await member("Usuário A", "matriz-a@demo.local", [empresaA]); AB = await member("Usuário AB", "matriz-ab@demo.local", [empresaA, empresaB]);
});
...
describe("matriz cross-farm: leituras, detalhes, contadores, dashboards, relatórios e recursos genéricos", () => {
  it("FINANCEIRO: títulos (lista/detalhe), totais do dashboard e relatório respeitam member_farms", async () => {
    const pay = (farm: string, n: string) => ({ empresa_id: farm, number: n, person_id: I.provider, amount: "100.00", emission_date: "2026-09-01", due_date: "2026-01-15", note: "Matriz", apportionment: [{ financial_category_id: I.category, cost_center_id: I.costCenter, percentage: "100" }] });
    const ta = await mk("/api/financial/payables", pay(empresaA, "MX-A")); const tb = await mk("/api/financial/payables", pay(empresaB, "MX-B"));
    ...
    // empresa_ids pedido fora do escopo não amplia: USER_A pedindo B recebe vazio, não "todas"
    const forced = j(await get(`/api/dashboards/financial?empresa_ids=${empresaB}`, A)).overdue as { n: number }[]; expect(forced.reduce((s, x) => s + Number(x.n), 0)).toBe(0);
  });
  it("VENDAS: orçamentos (lista/detalhe) e curva ABC respeitam member_farms", async () => {
    const doc = (farm: string, wh: string) => ({ empresa_id: farm, document_date: "2026-09-10", client_id: I.client, items: [{ product_id: I.product2, warehouse_id: wh, quantity: "1", unit_price: "5" }] });
    const sa = await mk("/api/sales/budgets", doc(empresaA, whA)); const sb = await mk("/api/sales/budgets", doc(empresaB, whB));
  ...
  it("ESTOQUE: entradas (lista/detalhe), saldo por armazém e movimentações respeitam member_farms", async () => {
    const entry = (farm: string, wh: string) => ({ empresa_id: farm, entry_date: "2026-09-01", items: [{ product_id: I.product, quantity: "7", unit_value: "3", warehouse_id: wh, financial_category_id: I.category, cost_center_id: I.costCenter }] });
    const ea = await mk("/api/stock/input-entries", entry(empresaA, whA)); const eb = await mk("/api/stock/input-entries", entry(empresaB, whB));
    ...
    const balA = j(await get("/api/stock/balances", A)).items as { warehouse_id: string }[]; expect(balA.some((x) => x.warehouse_id === I.warehouseEmpresa2)).toBe(false); expect(balA.some((x) => x.warehouse_id === I.warehouse)).toBe(true);
    expect((await get(`/api/stock/balances/${I.warehouseEmpresa2}/${I.product}`, A)).statusCode).toBe(404); expect((await get(`/api/stock/balances/${I.warehouseEmpresa2}/${I.product}`, OWNER)).statusCode).toBe(200);
    const movA = j(await get("/api/stock/movements", A)).items as { empresa_id: string }[]; expect(movA.every((m) => m.empresa_id === empresaA)).toBe(true); expect(movA.length).toBeGreaterThan(0);
    const movO = j(await get("/api/stock/movements", OWNER)).items as { empresa_id: string }[]; expect(movO.some((m) => m.empresa_id === empresaB)).toBe(true);
  ...
  it("ORDENS DE SERVIÇO: lista, detalhe, contadores (monitoramento) e atrasadas respeitam member_farms", async () => {
    const oa = await mk("/api/service-orders", { empresa_id: empresaA, order_date: "2026-09-01", description: "OS A", planned_end: "2026-01-01" }); const ob = await mk("/api/service-orders", { empresa_id: empresaB, order_date: "2026-09-01", description: "OS B", planned_end: "2026-01-01" });
  ...
  it("FROTA: bens (recurso genérico por id), manutenções e depreciação respeitam member_farms", async () => {
    ...
    const eqB = (await admin.query<{ id: string }>("insert into erp.equipments(organization_id,empresa_id,code,description,equipment_type,status,has_depreciation,acquisition_value,life_years,acquisition_date) values ($1,$2,'MXB','Trator matriz B','own','active',true,120000,10,'2025-01-01') returning id", [h.demo.orgId, empresaB])).rows[0]!.id;
    await admin.end();
    ...
    expect((await h.app.inject({ method: "POST", url: "/api/assets/depreciations/run", headers: A, payload: { period_month: "2026-08-01", empresa_id: empresaB } })).statusCode).toBe(403);
  });
  it("FÁBRICA DE RAÇÃO: produções (lista/detalhe) respeitam member_farms; id inválido/inexistente → 404 (UI-STAB-01)", async () => {
    const entry = (farm: string, wh: string) => ({ empresa_id: farm, entry_date: "2026-09-01", items: [{ product_id: I.product, quantity: "50", unit_value: "3", warehouse_id: wh, financial_category_id: I.category, cost_center_id: I.costCenter }] });
    await mk("/api/stock/input-entries", entry(empresaA, whA)); await mk("/api/stock/input-entries", entry(empresaB, whB));
    ...
    const batch = (farm: string, wh: string) => ({ empresa_id: farm, batch_date: "2026-09-02", formula_id: formula, origin_warehouse_id: wh, destination_warehouse_id: wh, quantity_produced: "2" });
    const fa = await mk("/api/stock/feed-batches", batch(empresaA, whA)); const fb = await mk("/api/stock/feed-batches", batch(empresaB, whB));
  ...
  it("BAIXA DIRETA: detalhe traz armazém do cabeçalho nas linhas (itens sem warehouse_id) e respeita member_farms (UI-STAB-01)", async () => {
    const entry = (farm: string, wh: string) => ({ empresa_id: farm, entry_date: "2026-09-01", items: [{ product_id: I.product, quantity: "10", unit_value: "3", warehouse_id: wh, financial_category_id: I.category, cost_center_id: I.costCenter }] });
    await mk("/api/stock/input-entries", entry(empresaA, whA)); await mk("/api/stock/input-entries", entry(empresaB, whB));
    const wo = (farm: string, wh: string) => ({ empresa_id: farm, writeoff_date: "2026-09-02", reason: "loss", warehouse_id: wh, justification: "matriz", items: [{ product_id: I.product, quantity: "1" }] });
    const wa = await mk("/api/stock/writeoffs", wo(empresaA, whA)); const wb = await mk("/api/stock/writeoffs", wo(empresaB, whB));
  ...
  it("PECUÁRIA: manejos, pesagens, animais e dashboard continuam corretos; localizar animal não vaza outra fazenda", async () => {
    const ha = await mk("/api/livestock/handlings", { empresa_id: empresaA, handling_type: "sanitary", handling_date: "2026-09-20", batch_id: I.batch, items: [{ animal_id: I.animal, quantity: "1" }] });
    const hb = await mk("/api/livestock/handlings", { empresa_id: empresaB, handling_type: "sanitary", handling_date: "2026-09-20", batch_id: I.batch, items: [{ animal_id: I.animal, quantity: "1" }] });
    await matrix("handlings", "/api/livestock/handlings", ha, hb);
    const wa = await mk("/api/livestock/weighings", { empresa_id: empresaA, weighing_date: "2026-09-20", batch_id: I.batch, items: [{ animal_id: I.animal, weight: "300" }] });
    const wb = await mk("/api/livestock/weighings", { empresa_id: empresaB, weighing_date: "2026-09-21", batch_id: I.batch, items: [{ animal_id: I.animal, weight: "301" }] });
    await matrix("weighings", "/api/livestock/weighings", wa, wb);
    const admin = createPool(TEST_URL, { max: 1 });
    const animalB = (await admin.query<{ id: string }>("select id from erp.animals where organization_id=$1 and empresa_id=$2 and deleted_at is null limit 1", [h.demo.orgId, empresaB])).rows[0]?.id;
    await admin.end();
    ...
    const w = await h.app.inject({ method: "POST", url: "/api/livestock/weighings", headers: A, payload: { empresa_id: empresaB, weighing_date: "2026-09-22", items: [{ animal_id: I.animal, weight: "1" }] } }); expect([403, 422]).toContain(w.statusCode);
  });
  it("RECURSOS GENÉRICOS e LOOKUPS: lista, getOne, options e distinct de recurso farmScoped respeitam member_farms", async () => {
    ...
    const optA = j(await get("/api/resources/warehouses/options", A)) as unknown as { id: string }[]; expect(optA.some((o) => o.id === I.warehouseEmpresa2)).toBe(false); expect(optA.some((o) => o.id === I.warehouse)).toBe(true);
    const optO = j(await get("/api/resources/warehouses/options", OWNER)) as unknown as { id: string }[]; expect(optO.some((o) => o.id === I.warehouseEmpresa2)).toBe(true);
    const distA = j(await get("/api/resources/warehouses/distinct?field=empresa_id", A)) as unknown as { value: string }[]; expect(distA.some((d) => d.value === empresaB)).toBe(false);
  ...
  it("NÃO REGRESSÃO: owner e usuário multi-fazenda continuam operando; permissão funcional continua sendo verificada", async () => {
    ...
    expect((await get("/api/financial/payables", { ...AB, "x-empresa-id": empresaB })).statusCode).toBe(200);
  });
});
