...
import { escoposDeTodosOsModulos, harness, ids, type Harness } from "./setup.js";
...
describe("saúde e autenticação", () => {
  ...
  it("contexto retorna empresas e permissões", async () => { const r = await h.app.inject({ method: "GET", url: "/api/auth/context", headers: h.headers() }); expect(r.statusCode).toBe(200); const c = j(r) as { empresas: unknown[]; farms?: unknown; permissions: string[]; isOwner: boolean }; expect(c.empresas.length).toBe(2); expect(c.farms, "o apelido de resposta saiu em PRE-BASE2-05B").toBeUndefined(); expect(c.isOwner).toBe(true); expect(c.permissions.length).toBeGreaterThan(600); });
});
...
describe("estoque: ledger, documentos e concorrência", () => {
  it("estoque inicial → requisição → saldo e custo médio; saldo negativo é bloqueado", async () => {
    const ob = await h.app.inject({ method: "POST", url: "/api/stock/opening-balances", headers: h.headers(), payload: { empresa_id: I.empresa, warehouse_id: I.warehouse, product_id: I.product, quantity: "100", unit_value: "10" } });
    expect(st(ob, 201)).toBe(201);
    const entry = await h.app.inject({ method: "POST", url: "/api/stock/input-entries", headers: h.headers(), payload: { empresa_id: I.empresa, entry_date: "2026-09-01", items: [{ product_id: I.product, quantity: "100", unit_value: "20", warehouse_id: I.warehouse, financial_category_id: I.category, cost_center_id: I.costCenter }] } });
    ...
    const req = await h.app.inject({ method: "POST", url: "/api/stock/requisitions", headers: h.headers(), payload: { empresa_id: I.empresa, requisition_date: "2026-09-02", requester_person_id: I.employee, items: [{ warehouse_id: I.warehouse, product_id: I.product, quantity: "50", cost_center_id: I.costCenter }] } });
    expect(req.statusCode).toBe(201); expect(j(req).total_amount).toBe("750.00");
    const over = await h.app.inject({ method: "POST", url: "/api/stock/requisitions", headers: h.headers(), payload: { empresa_id: I.empresa, requisition_date: "2026-09-02", items: [{ warehouse_id: I.warehouse, product_id: I.product, quantity: "500" }] } });
  ...
  it("transação atômica: falha no 2º item não deixa o 1º lançado", async () => {
    ...
    const r = await h.app.inject({ method: "POST", url: "/api/stock/requisitions", headers: h.headers(), payload: { empresa_id: I.empresa, requisition_date: "2026-09-02", items: [{ warehouse_id: I.warehouse, product_id: I.product, quantity: "10" }, { warehouse_id: I.warehouse, product_id: I.product2, quantity: "999999" }] } });
  ...
  it("idempotência: mesma chave não duplica o documento; corpo diferente conflita", async () => {
    const payload = { empresa_id: I.empresa, requisition_date: "2026-09-03", items: [{ warehouse_id: I.warehouse, product_id: I.product, quantity: "5" }] };
  ...
  it("concorrência: 10 requisições simultâneas de 20 un. com saldo 145 → exatamente 7 sucessos", async () => {
    const results = await Promise.all(Array.from({ length: 10 }, () => h.app.inject({ method: "POST", url: "/api/stock/requisitions", headers: h.headers(), payload: { empresa_id: I.empresa, requisition_date: "2026-09-05", items: [{ warehouse_id: I.warehouse, product_id: I.product, quantity: "20" }] } })));
  ...
  it("cancelamento estorna o ledger", async () => {
    const dev = await h.app.inject({ method: "POST", url: "/api/stock/devolutions", headers: h.headers(), payload: { empresa_id: I.empresa, devolution_date: "2026-09-06", items: [{ warehouse_id: I.warehouse, product_id: I.product, quantity: "95" }] } });
  ...
  it("transferência entre fazendas com financeiro gera par de títulos", async () => {
    await h.app.inject({ method: "POST", url: "/api/stock/opening-balances", headers: h.headers(), payload: { empresa_id: I.empresa, warehouse_id: I.warehouse, product_id: I.product2, quantity: "1000", unit_value: "2" } });
    const t = await h.app.inject({ method: "POST", url: "/api/stock/transfers", headers: h.headers(), payload: { kind: "farm", transfer_date: "2026-09-07", empresa_origem_id: I.empresa, origin_warehouse_id: I.warehouse, empresa_destino_id: I.empresa2, destination_warehouse_id: I.warehouseEmpresa2, items: [{ product_id: I.product2, quantity: "100" }], generate_financial: true, plan: { installments: 2, first_due_date: "2026-10-01", mode: "interval", interval_days: 30 }, income_apportionment: [{ financial_category_id: I.incomeCategory, cost_center_id: I.costCenter, percentage: "100" }], expense_apportionment: [{ financial_category_id: I.category, cost_center_id: I.costCenter, percentage: "100" }] } });
    ...
    expect(j(await h.app.inject({ method: "GET", url: `/api/stock/balances/${I.warehouseEmpresa2}/${I.product2}`, headers: h.headers() })).quantity).toBe("100.0000");
    const same = await h.app.inject({ method: "POST", url: "/api/stock/transfers", headers: h.headers(), payload: { kind: "warehouse", transfer_date: "2026-09-07", empresa_origem_id: I.empresa, origin_warehouse_id: I.warehouse, destination_warehouse_id: I.warehouse, items: [{ product_id: I.product2, quantity: "1" }] } });
...
describe("financeiro: títulos, baixas, movimentos, congelamento", () => {
  ...
  it("baixa em lote com movimento único e baixa cruzada", async () => {
    ...
    const b = j(await h.app.inject({ method: "POST", url: "/api/financial/payables", headers: h.headers(), payload: { empresa_id: I.empresa, number: "L2", person_id: I.provider, amount: "200", emission_date: "2026-09-01", due_date: "2026-09-30", note: "lote", apportionment: [{ financial_category_id: I.category, cost_center_id: I.costCenter, percentage: "100" }] } })).id;
    ...
    const recv = j(await h.app.inject({ method: "POST", url: "/api/financial/receivables", headers: h.headers(), payload: { empresa_id: I.empresa, number: "R1", person_id: I.provider, amount: "150", emission_date: "2026-09-01", due_date: "2026-09-30", note: "cruzada", apportionment: [{ financial_category_id: I.incomeCategory, cost_center_id: I.costCenter, percentage: "100" }] } })).id;
...
describe("suprimentos: workflow completo", () => {
  ...
  it("solicitação → ciência → cotação → seleção → aprovação → compra → recebimento (exige NF) → finalização", async () => {
    const r = await h.app.inject({ method: "POST", url: "/api/supply/requests", headers: h.headers(), payload: { empresa_id: I.empresa, request_date: "2026-09-01", request_type: "product", description: "Compra de sal", justification: "Reposição", priority: "high", items: [{ product_id: I.product, description: "Sal mineral", quantity: "10", reference_value: "5" }] } });
  ...
  it("controle otimista (version) e operador sem permissão de aprovar", async () => {
    const r2 = j(await h.app.inject({ method: "POST", url: "/api/supply/requests", headers: h.headers(), payload: { empresa_id: I.empresa, request_date: "2026-09-02", request_type: "service", description: "Serviço", justification: "x", items: [{ description: "Conserto", quantity: "1", amount: "300" }] } })).id;
...
describe("anexos por registro", () => {
  it("envia, lista, baixa (inline/download), audita e exclui; valida tipo, assinatura e permissão", async () => {
    ...
    const up = await h.app.inject({ method: "POST", url: "/api/attachments", headers: h.headers(), payload: { entity: "empresas", entity_id: I.empresa, file_name: "contrato: 2026.pdf", mime_type: "application/pdf", description: "Contrato", data_base64: pdf } });
    expect(up.statusCode, up.body).toBe(201); const id = j(up).id as string; expect(j(up).file_name).toBe("contrato__2026.pdf");
    const list = j(await h.app.inject({ method: "GET", url: `/api/attachments?entity=empresas&entity_id=${I.empresa}`, headers: h.headers() })) as { items: { id: string; size_bytes: string | number }[] };
    ...
    expect((await h.app.inject({ method: "POST", url: "/api/attachments", headers: h.headers(), payload: { entity: "empresas", entity_id: I.empresa, file_name: "x.exe", mime_type: "application/x-msdownload", data_base64: pdf } })).statusCode).toBe(422);
    expect((await h.app.inject({ method: "POST", url: "/api/attachments", headers: h.headers(), payload: { entity: "empresas", entity_id: I.empresa, file_name: "x.png", mime_type: "image/png", data_base64: pdf } })).statusCode).toBe(422);
    // operador pode ver/enviar, não pode excluir
    expect((await h.app.inject({ method: "GET", url: `/api/attachments?entity=empresas&entity_id=${I.empresa}`, headers: h.opHeaders() })).statusCode).toBe(403); // attachments.view não contorna farms.view (permissão do pai)
    ...
    const aud = j(await h.app.inject({ method: "GET", url: `/api/admin/audit?entity=empresas&entity_id=${I.empresa}`, headers: h.headers() })) as { items: { action: string }[] };
...
describe("vendas, frota, RH e pecuária", () => {
  it("orçamento → pedido → venda confirmada gera baixa de estoque e conta a receber", async () => {
    const b = await h.app.inject({ method: "POST", url: "/api/sales/budgets", headers: h.headers(), payload: { empresa_id: I.empresa, document_date: "2026-09-10", client_id: I.client, items: [{ product_id: I.product2, warehouse_id: I.warehouse, quantity: "10", unit_price: "5", discount_percent: "10" }], freight: "20" } });
  ...
  it("abastecimento baixa combustível do estoque e atualiza horímetro; depreciação mensal roda idempotente", async () => {
    ...
    await h.app.inject({ method: "POST", url: "/api/stock/opening-balances", headers: h.headers(), payload: { empresa_id: I.empresa, warehouse_id: I.warehouse, product_id: diesel.id, quantity: "500", unit_value: "6" } });
    const s = await h.app.inject({ method: "POST", url: "/api/fleet/fuel-supplies", headers: h.headers(), payload: { empresa_id: I.empresa, supply_date: "2026-09-10", equipment_id: I.equipment, warehouse_id: I.warehouse, product_id: diesel.id, quantity: "50", hour_meter: "1200" } });
  ...
  it("apuração mensal consolida salário, adiantamento e faltas e gera financeiro", async () => {
    ...
    const adv = await h.app.inject({ method: "POST", url: "/api/hr/advances", headers: h.headers(), payload: { empresa_id: I.empresa, advance_date: "2026-09-05", person_id: I.employee, amount: "300", due_date: "2026-09-20", financial_category_id: I.category, cost_center_id: I.costCenter } });
    expect(adv.statusCode).toBe(201);
    const e = await h.app.inject({ method: "POST", url: "/api/hr/earnings/calculate", headers: h.headers(), payload: { empresa_id: I.empresa, reference_month: "2026-09-01" } });
  ...
  it("pecuária: pesagem calcula GMD, venda de animal gera receita, compra cria processamento, manejo sanitário aplica carência", async () => {
    const w1 = await h.app.inject({ method: "POST", url: "/api/livestock/weighings", headers: h.headers(), payload: { empresa_id: I.empresa, weighing_date: "2026-09-01", batch_id: I.batch, items: [{ animal_id: I.animal, weight: "280" }] } });
    expect(w1.statusCode).toBe(201);
    const w2 = await h.app.inject({ method: "POST", url: "/api/livestock/weighings", headers: h.headers(), payload: { empresa_id: I.empresa, weighing_date: "2026-09-11", batch_id: I.batch, items: [{ animal_id: I.animal, weight: "290" }] } });
    ...
    const san = await h.app.inject({ method: "POST", url: "/api/livestock/handlings", headers: h.headers(), payload: { empresa_id: I.empresa, handling_type: "sanitary", handling_date: "2026-09-12", batch_id: I.batch, product_id: I.productLot, warehouse_id: null, dose: "2", items: [{ animal_id: I.animal, quantity: "1" }] } });
    ...
    const nut = await h.app.inject({ method: "POST", url: "/api/livestock/handlings", headers: h.headers(), payload: { empresa_id: I.empresa, handling_type: "nutrition", handling_date: "2026-09-12", batch_id: I.batch, items: [{ animal_id: I.animal, quantity: "1" }] } });
    ...
    const again = await h.app.inject({ method: "POST", url: "/api/livestock/movements", headers: h.headers(), payload: { empresa_id: I.empresa, movement_type: "death", movement_date: "2026-09-16", items: [{ animal_id: I.animal }] } });
    expect(j(again).error!.code).toBe("ANIMAL_NOT_ACTIVE");
    const buy = await h.app.inject({ method: "POST", url: "/api/livestock/movements", headers: h.headers(), payload: { empresa_id: I.empresa, movement_type: "purchase", movement_date: "2026-09-17", person_id: I.provider, batch_id: I.batch, generate_financial: true, items: [{ category_id: I.speciesCategory, quantity: 5, weight: "200", unit_value: "2000" }] } });
...
describe("segurança: escopo de fazenda em leituras por id (mesma organização, mesma permissão, fazenda não autorizada)", () => {
  it("usuário restrito à Fazenda A lê registros de A e recebe 404 para B; proprietário (todas as fazendas) lê ambos", async () => {
    ...
    const mem = await h.app.inject({ method: "POST", url: "/api/admin/members", headers: h.headers(), payload: { name: "Restrito A", email: "restrito-a@demo.local", password: "Restrito@12345", role_id: j(role).id, escopos_empresas: escoposDeTodosOsModulos([I.empresa]) } });
    ...
    const hA = await mk("/api/livestock/handlings", { empresa_id: I.empresa, handling_type: "sanitary", handling_date: "2026-09-20", batch_id: I.batch, items: [{ animal_id: I.animal, quantity: "1" }] });
    const hB = await mk("/api/livestock/handlings", { empresa_id: I.empresa2, handling_type: "sanitary", handling_date: "2026-09-20", batch_id: I.batch, items: [{ animal_id: I.animal, quantity: "1" }] });
    const wA = await mk("/api/livestock/weighings", { empresa_id: I.empresa, weighing_date: "2026-09-20", batch_id: I.batch, items: [{ animal_id: I.animal, weight: "300" }] });
    const wB = await mk("/api/livestock/weighings", { empresa_id: I.empresa2, weighing_date: "2026-09-21", batch_id: I.batch, items: [{ animal_id: I.animal, weight: "301" }] });
    const sA = await mk("/api/fleet/fuel-supplies", { empresa_id: I.empresa, supply_date: "2026-09-20", equipment_id: I.equipment, product_id: diesel.id, quantity: "5" });
    const sB = await mk("/api/fleet/fuel-supplies", { empresa_id: I.empresa2, supply_date: "2026-09-20", equipment_id: I.equipment, product_id: diesel.id, quantity: "5" });
    ...
    for (const [a, b, base] of [[hA, hB, "/api/livestock/handlings"], [wA, wB, "/api/livestock/weighings"], [sA, sB, "/api/fleet/fuel-supplies"]] as const) {
      ...
      expect((await get(`${base}/${b}`, { ...rh, "x-empresa-id": I.empresa2 })).statusCode).toBe(403);
      // listagem do restrito nunca traz registro de B
      const ls = j(await get(base, rh)) as { items: { id: string; empresa_id: string }[] };
      expect(ls.items.some((r) => r.id === b)).toBe(false); expect(ls.items.every((r) => r.empresa_id === I.empresa)).toBe(true); expect(ls.items.some((r) => r.id === a)).toBe(true);
      ...
      const selA = j(await get(base, h.headers({ "x-empresa-id": I.empresa }))) as { items: { id: string }[] }; expect(selA.items.some((r) => r.id === b)).toBe(false);
    }
    // endpoints preexistentes do mesmo fluxo (animal, movimentação, manutenção) seguem o mesmo escopo
    const mB = await mk("/api/livestock/movements", { empresa_id: I.empresa2, movement_type: "purchase", movement_date: "2026-09-22", person_id: I.provider, batch_id: I.batch, items: [{ category_id: I.speciesCategory, quantity: 1, weight: "200", unit_value: "1000" }] });
    ...
    const animalB = j(await get(`/api/livestock/animals?empresa_id=${I.empresa2}&pageSize=1`, h.headers())).items![0] as { id: string } | undefined;
