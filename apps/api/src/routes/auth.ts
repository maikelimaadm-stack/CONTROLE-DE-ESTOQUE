...
import { resolveLocale } from "@agro/platform";
...
export default async function authRoutes(app: FastifyInstance) {
  ...
  app.get("/auth/context", async (req) => runService(app, req, null, async (ctx) => {
    ...
    const org = await ctx.tx.query<{ name: string; parameters: unknown; default_language: string }>("select name, parameters, default_language from erp.organizations where id=$1", [ctx.orgId]);
    const userLang = await ctx.tx.query<{ language: string | null }>("select language from erp.users where id=$1", [ctx.user.id]);
    ...
    const language = { organization: org.rows[0]?.default_language ?? null, user: userLang.rows[0]?.language ?? null, effective: resolveLocale({ user: userLang.rows[0]?.language ?? null, organization: org.rows[0]?.default_language ?? null }) };
    // `farms` é a lista de EMPRESAS visíveis (docs/MULTI-COMPANY-CONTRACT.md); o nome do campo migra em PRE-BASE2-03.
    return { user: ctx.user, organization: { id: ctx.orgId, name: org.rows[0]?.name, parameters: org.rows[0]?.parameters ?? {} }, isOwner: ctx.membership.isOwner, farms: visibleFarms, permissions: perms, favorites: fav.rows, unreadNotifications: Number(unread.rows[0]?.n ?? 0), canViewUsers: hasPermission(ctx, "users.view"), language };
  }));
}
