...
export default async function financialRoutes(app: FastifyInstance) {
  for (const dir of ["payable", "receivable"] as const) {
    ...
    app.put(`${base}/:id`, async (req) => runService(app, req, permOf(dir, "edit"), async (ctx) => {
      ...
      if (!cur.rows[0]) throw notFound("Título"); if (cur.rows[0].status === "cancelled") throw err("ALREADY_CANCELLED", "Título cancelado");
    ...
    app.post(`${base}/:id/cancel`, async (req) => runService(app, req, permOf(dir, "delete"), async (ctx) => {
      ...
      if (!cur.rows[0]) throw notFound("Título"); if (cur.rows[0].status === "cancelled") throw err("ALREADY_CANCELLED", "Já cancelado"); if (D(cur.rows[0].paid_amount).gt(0)) throw err("CONFLICT", "Título com baixa: cancele a baixa antes");
      await assertPeriodOpen(ctx.tx, ctx.orgId, cur.rows[0].farm_id, cur.rows[0].emission_date);
    ...
    app.post(`${base}/cancel-batch`, async (req) => runService(app, req, permOf(dir, "delete"), async (ctx) => { const d = z.object({ ids: z.array(uuid).min(1) }).parse(req.body); let n = 0; for (const id of d.ids) { const r = await ctx.tx.query("update erp.financial_titles set status='cancelled', version=version+1 where id=$1 and organization_id=$2 and direction=$3 and status<>'cancelled' and paid_amount=0", [id, ctx.orgId, dir]); n += r.rowCount ?? 0; } return { cancelled: n, skipped: d.ids.length - n }; }));
    ...
    app.post(`${base}/settle-batch`, async (req, reply) => reply.status(201).send(await runService(app, req, permOf(dir, "settle"), async (ctx) => {
      ...
      return (await idempotent(ctx.tx, ctx.orgId, idem(req), d, async () => {
        ...
        for (const id of d.ids) { const bal = (await ctx.tx.query<{ balance: string; direction: string; farm_id: string; number: string }>("select balance, direction, farm_id, number from erp.financial_titles where id=$1 and organization_id=$2 and status in ('open','partially_paid') for update", [id, ctx.orgId])).rows[0]; if (!bal) continue; total = total.plus(bal.balance); }
    ...
    app.post(`${base}/:id/settlements/:sid/cancel`, async (req) => runService(app, req, permOf(dir, "cancel_settlement"), async (ctx) => {
      ...
      const t = await ctx.tx.query<{ farm_id: string }>("select farm_id from erp.financial_titles where id=$1", [id]); await assertPeriodOpen(ctx.tx, ctx.orgId, t.rows[0]!.farm_id, s.rows[0].settlement_date);
  ...
  async function settle(ctx: ServiceCtx, titleId: string, d: { settlement_date: string; settlement_kind: "bank_movement" | "cross_settlement" | "advance_compensation"; bank_account_id?: string | null; cross_title_id?: string | null; amount: string; discount?: string; penalty?: string; interest?: string; increase?: string; foreign_amount?: string | null; ptax_rate?: string | null; exchange_adjustment?: string; note?: string | null; movement_mode: "separate" | "single"; shared_movement_id?: string | null }) {
    ...
    const title = t.rows[0]; if (!title) throw notFound("Título"); if (title.status === "cancelled") throw err("ALREADY_CANCELLED", "Título cancelado"); if (title.status === "paid") throw err("ALREADY_CONFIRMED", "Título já baixado");
  ...
  app.get("/financial/bank-movements", async (req) => runService(app, req, "bank_movements.view", async (ctx) => {
    ...
    if (f.farm_id) add("m.farm_id=?", f.farm_id); else if (ctx.farmId) add("(m.farm_id=? or m.farm_id is null)", ctx.farmId);
    const w = where.join(" and ");
  ...
  app.put("/financial/bank-movements/:id", async (req) => runService(app, req, "bank_movements.edit", async (ctx) => {
    ...
    const cur = await ctx.tx.query<{ source_type: string | null; farm_id: string | null; movement_date: string; status: string }>("select source_type, farm_id, movement_date, status from erp.bank_movements where id=$1 and organization_id=$2 for update", [id, ctx.orgId]); if (!cur.rows[0]) throw notFound("Movimento");
  ...
  app.post("/financial/bank-movements/:id/cancel", async (req) => runService(app, req, "bank_movements.delete", async (ctx) => {
    const { id } = req.params as { id: string };
    const cur = await ctx.tx.query<{ status: string; farm_id: string | null; movement_date: string; transfer_pair_id: string | null }>("select status, farm_id, movement_date, transfer_pair_id from erp.bank_movements where id=$1 and organization_id=$2 for update", [id, ctx.orgId]); if (!cur.rows[0]) throw notFound("Movimento"); if (cur.rows[0].status === "cancelled") throw err("ALREADY_CANCELLED", "Já cancelado");
  ...
  app.get("/financial/ofx-imports/:id", async (req) => runService(app, req, "ofx_imports.view", async (ctx) => { const { id } = req.params as { id: string }; const i = await ctx.tx.query("select * from erp.ofx_imports where id=$1 and organization_id=$2", [id, ctx.orgId]); if (!i.rows[0]) throw notFound(); const t = await ctx.tx.query("select t.*, m.code as movement_code, m.note as movement_note from erp.ofx_transactions t left join erp.bank_movements m on m.id=t.bank_movement_id where t.import_id=$1 order by t.posted_date", [id]); return { ...i.rows[0], transactions: t.rows }; }));
  ...
  app.get("/financial/budget-plannings/:id/values", async (req) => runService(app, req, "budget_plannings.view", async (ctx) => {
    const { id } = req.params as { id: string }; const p = await ctx.tx.query<{ year: number; farm_id: string | null }>("select year, farm_id from erp.budget_plannings where id=$1 and organization_id=$2", [id, ctx.orgId]); if (!p.rows[0]) throw notFound();
  ...
  app.put("/financial/budget-plannings/:id/values", async (req) => runService(app, req, "budget_plannings.edit", async (ctx) => {
    ...
    const p = await ctx.tx.query("select 1 from erp.budget_plannings where id=$1 and organization_id=$2", [id, ctx.orgId]); if (!p.rowCount) throw notFound();
