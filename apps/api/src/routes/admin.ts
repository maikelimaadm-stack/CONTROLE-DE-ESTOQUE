...
export default async function adminRoutes(app: FastifyInstance) {
  ...
  app.post("/admin/notifications/refresh", async (req) => runService(app, req, null, async (ctx) => {
    ...
    for (const p of pend.rows) await add("purchase_pending", `Compras nº ${p.code} pendente há ${p.days} dia(s)`, null, `/suprimentos/solicitacoes/${p.id}`, p.current_responsible_user_id);
    ...
    if (Number(due.rows[0]!.n) > 0) await add("title_due", `${due.rows[0]!.n} título(s) vencendo nos próximos 3 dias`, null, "/financeiro/contas-a-pagar?due_soon=1");
    ...
    for (const b of bday.rows) await add("birthday", `${b.name} está fazendo aniversário hoje`, null, "/cadastros/pessoas");
