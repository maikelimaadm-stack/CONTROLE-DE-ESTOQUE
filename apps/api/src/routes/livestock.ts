...
export default async function livestockRoutes(app: FastifyInstance) {
  ...
  app.get("/livestock/animals", async (req) => runService(app, req, "animals.view", async (ctx) => {
    ...
    where.push(...empresaScope(ctx, "a", params, { ignoreSelected: true }));
  ...
  app.post("/livestock/animals", async (req, reply) => reply.status(201).send(await runService(app, req, "animals.create", async (ctx) => {
    const d = animalSchema.parse(req.body); await exigirEmpresaDeLancamento(ctx, d.farm_id);
  ...
  app.put("/livestock/animals/:id", async (req) => runService(app, req, "animals.edit", async (ctx) => {
    ...
    const cur = await ctx.tx.query<{ farm_id: string }>("select farm_id from erp.animals where id=$1 and organization_id=$2 and deleted_at is null for update", [id, ctx.orgId]); if (!cur.rows[0]) throw notFound("Animal"); await exigirEmpresaVisivel(ctx, cur.rows[0].farm_id, "Animal");
  ...
  app.get("/livestock/herd-lots", async (req) => runService(app, req, "animals.view", async (ctx) => { const r = await consultaEscopada(ctx, "select h.*, c.name as category_name, b.name as breed_name, bt.description as batch_name, f.name as farm_name from erp.herd_lots h join erp.animal_categories c on c.id=h.category_id left join erp.breeds b on b.id=h.breed_id left join erp.batches bt on bt.id=h.batch_id join erp.farms f on f.id=h.farm_id where h.organization_id=$1 and h.quantity>0 and {{escopo:h.farm_id}} order by f.name, c.name", [ctx.orgId]); return { items: r.rows, total: r.rowCount, total_quantity: r.rows.reduce((a, x) => a + (x as { quantity: number }).quantity, 0) }; }));
  // Localiza animal (busca por identificação em todas as fazendas)
  // Localiza Animal: o escopo do módulo pecuária entra no próprio SQL (sem lista de empresas em memória)
  app.get("/livestock/locate", async (req) => runService(app, req, "locate_animals.view", async (ctx) => {
    const { search } = req.query as { search?: string }; if (!search) return { items: [] };
    const params: unknown[] = [ctx.orgId, `%${search}%`];
    const escopo = empresaScopeSql(ctx, "a", params, { ignoreSelected: true });
    const r = await ctx.tx.query(`select a.id, a.status, i.value as identification, t.name as identification_type, f.name as farm_name, bt.description as batch_name, gm.description as module_name, ar.name as area_name, c.name as category_name from erp.animal_identifications i join erp.animals a on a.id=i.animal_id join erp.identification_types t on t.id=i.identification_type_id join erp.farms f on f.id=a.farm_id left join erp.batches bt on bt.id=a.batch_id left join erp.grazing_modules gm on gm.id=bt.grazing_module_id left join erp.areas ar on ar.id=bt.area_id join erp.animal_categories c on c.id=a.category_id where i.organization_id=$1 and i.value ilike $2${escopo} limit 50`, params);
    return { items: r.rows };
  }));
  ...
  app.post("/livestock/transfers/animals-to-batch", async (req, reply) => reply.status(201).send(await runService(app, req, "animal_batch_transfer.create", async (ctx) => {
    const d = z.object({ farm_id: uuid, movement_date: date, animal_ids: z.array(uuid).min(1), destination_batch_id: uuid, note: z.string().optional().nullable() }).parse(req.body); await exigirEmpresaDeLancamento(ctx, d.farm_id);
  ...
  app.post("/livestock/transfers/group-batches", async (req, reply) => reply.status(201).send(await runService(app, req, "batch_grouping.create", async (ctx) => {
    const d = z.object({ farm_id: uuid, movement_date: date, source_batch_ids: z.array(uuid).min(1), destination_batch_id: uuid, close_sources: z.boolean().default(true) }).parse(req.body); await exigirEmpresaDeLancamento(ctx, d.farm_id);
  ...
  app.post("/livestock/transfers/batch-to-module-area", async (req, reply) => reply.status(201).send(await runService(app, req, "batch_module_area_transfer.create", async (ctx) => {
    const d = z.object({ farm_id: uuid, movement_date: date, batch_id: uuid, grazing_module_id: uuid.optional().nullable(), area_id: uuid.optional().nullable(), corral_id: uuid.optional().nullable(), note: z.string().optional().nullable() }).parse(req.body); await exigirEmpresaDeLancamento(ctx, d.farm_id);
  ...
  app.post("/livestock/transfers/:id/process", async (req) => runService(app, req, "batch_farm_transfer.process", async (ctx) => {
    ...
    if (!(await empresaPermitida(ctx, m.rows[0].destination_farm_id))) throw err("PERMISSION_DENIED", "Sem acesso à empresa de destino");
  ...
  app.post("/livestock/evolution/run", async (req, reply) => reply.status(201).send(await runService(app, req, "herd_evolution.create", async (ctx) => {
    const d = z.object({ farm_id: uuid, movement_date: date.default(todayISO()) }).parse(req.body); await exigirEmpresaDeLancamento(ctx, d.farm_id);
  ...
  app.post("/livestock/weighings", async (req, reply) => reply.status(201).send(await runService(app, req, "weighings.create", async (ctx) => {
    const d = z.object({ farm_id: uuid, weighing_date: date, batch_id: uuid.optional().nullable(), responsible: z.string().optional().nullable(), note: z.string().optional().nullable(), items: z.array(z.object({ animal_id: uuid, weight: dec })).min(1) }).parse(req.body); await exigirEmpresaDeLancamento(ctx, d.farm_id);
    return (await idempotent(ctx.tx, ctx.orgId, idem(req), d, async () => {
  ...
  app.post("/livestock/handlings", async (req, reply) => reply.status(201).send(await runService(app, req, "nutritions.create", async (ctx) => {
    const d = handlingSchema.parse(req.body); await exigirEmpresaDeLancamento(ctx, d.farm_id);
  ...
  app.post("/livestock/processings/:id/process", async (req) => runService(app, req, "processings.process", async (ctx) => {
    ...
    const p = await ctx.tx.query<{ status: string; farm_id: string; processed_quantity: number; expected_quantity: number }>("select status, farm_id, processed_quantity, expected_quantity from erp.processings where id=$1 and organization_id=$2 for update", [id, ctx.orgId]); if (!p.rows[0]) throw notFound(); await exigirEmpresaVisivel(ctx, p.rows[0].farm_id, "Processamento"); if (["finished", "cancelled"].includes(p.rows[0].status)) throw err("ALREADY_CONFIRMED", "Processamento encerrado");
  ...
  app.post("/feedlot/diet-batches", async (req, reply) => reply.status(201).send(await runService(app, req, "diet_batches.create", async (ctx) => {
    const d = z.object({ farm_id: uuid, batch_date: date, diet_id: uuid, warehouse_id: uuid, equipment_id: uuid.optional().nullable(), quantity_kg: dec }).parse(req.body); await exigirEmpresaDeLancamento(ctx, d.farm_id);
    return (await idempotent(ctx.tx, ctx.orgId, idem(req), d, async () => {
