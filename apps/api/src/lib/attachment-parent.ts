...
function byId(table: string, opts: { softDelete?: boolean; shared?: boolean }) {
  return async (ctx: ServiceCtx, id: string) => {
    ...
    const r = await ctx.tx.query(`select * from erp.${table} where id=$1 and ${org}${opts.softDelete ? " and deleted_at is null" : ""}`, [id, ctx.orgId]);
...
const EXPLICIT: Record<string, ParentRule> = {
  service_orders: { kind: "farm", viewPerm: "service_orders.view", load: byId("service_orders", { softDelete: true }), origin: "explicit" },
  purchase_requests: { kind: "farm", viewPerm: "purchase_requests.view", load: byId("purchase_requests", { softDelete: true }), origin: "explicit" },
  weighings: { kind: "farm", viewPerm: "weighings.view", load: byId("weighings", { softDelete: true }), origin: "explicit" },
  animal_handlings: { kind: "farm", viewPerm: (row) => permissaoManejo(String(row["handling_type"] ?? ""), "view"), load: byId("animal_handlings", { softDelete: true }), origin: "explicit" },
