...
export function B1Field({ label, required, error, help, span = 3, disabled, locked, hasValue = true, multiline, open, children, className, flex, idDoControle, testId }: { label: string; /** id já posto na caixa por quem a desenha (partes de um campo de busca): o rótulo aponta para ele, sem clonar */ idDoControle?: string; testId?: string; required?: boolean; error?: string; help?: string; span?: number; /** modo visualização */ disabled?: boolean; /** travado em edição (somente leitura) */ locked?: boolean; /** rótulo pequeno no topo e valor na base; vazio = rótulo centralizado */ hasValue?: boolean; multiline?: boolean; /** seletor/calendário aberto */ open?: boolean; children: React.ReactNode; className?: string; flex?: boolean }) {
