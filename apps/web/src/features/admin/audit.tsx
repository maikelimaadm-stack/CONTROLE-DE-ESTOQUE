...
export function AuditPanel() {
  ...
  return <Card><CardHeader title="Auditoria" subtitle="Trilha imutável de criações, alterações, cancelamentos, logins e ações de fluxo (antes/depois em JSON)." /><CardBody>
    ...
    <Drawer open={Boolean(row)} onOpenChange={() => setRow(null)} title="Detalhe do evento" description={row ? `${enumLabel("audit_action", row["action"])} · ${String(row["entity"] ?? "")} · ${dateTimeBR(row["created_at"] as string)}` : undefined} size="lg">{row && <div className="grid gap-2 text-xs">{([["Antes", "before"], ["Depois", "after"], ["Metadados", "metadata"]] as const).map(([rotulo, campo]) =>
      // três campos, três blocos: encadear `after ?? metadata` escondia os metadados de todo evento que
      // passou a gravar a foto do depois na coluna própria.
      <div key={campo}><h4 className="font-semibold">{rotulo}</h4><pre className="max-h-80 overflow-auto rounded bg-slate-50 p-2">{row[campo] == null ? "—" : JSON.stringify(row[campo], null, 2)}</pre></div>)}</div>}</Drawer>
  </CardBody></Card>;
}
