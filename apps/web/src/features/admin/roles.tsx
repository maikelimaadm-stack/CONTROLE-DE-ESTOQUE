...
export function RolesPanel() {
  ...
  return <Card><PageHeader inCard title="Perfis de Acesso" subtitle="Cada perfil concede permissões por recurso e ação; a autorização é aplicada no servidor em todas as rotas." actions={can("roles.create") && <Link href="/admin/perfis/new"><Button size="sm">Novo perfil</Button></Link>} /><CardBody>{q.isLoading ? <LoadingState /> : <DataTable rows={q.data?.items ?? []} onRowClick={(r) => router.push(`/admin/perfis/${r["id"]}`)} columns={[{ key: "name", label: "Perfil" }, { key: "description", label: "Descrição" }, { key: "permission_count", label: "Permissões", align: "right" }, { key: "member_count", label: "Usuários", align: "right" }, { key: "is_system", label: "Tipo", render: (r) => r["is_system"] ? <Badge tone="blue">Sistema</Badge> : <Badge>Personalizado</Badge> }, { key: "updated_at", label: "Atualizado", render: (r) => dateTimeBR(r["updated_at"] as string) }]} />}</CardBody></Card>;
}
