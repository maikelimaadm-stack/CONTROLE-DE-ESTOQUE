...
import { AcessoPorEmpresa, type EscopoEmpresa, resumoEscopos } from "./acesso-empresa";
interface Form { name: string; email: string; phone: string; password: string; role_id: string; escopos_empresas: EscopoEmpresa[]; is_active: boolean; boss_user_ids: string[] }
const blank: Form = { name: "", email: "", phone: "", password: "", role_id: "", escopos_empresas: [], is_active: true, boss_user_ids: [] };
export function UsersPanel() {
  const { can, ctx } = useAuth(); const tr = useTradutor(); const [search, setSearch] = React.useState(""); const [page, setPage] = React.useState(1); const [edit, setEdit] = React.useState<{ id?: string; f: Form } | null>(null);
  ...
  return <Card><CardHeader title="Usuários da Organização" subtitle="Vínculo usuário × perfil (o quê) × acesso por empresa e módulo (onde)." actions={<><Input placeholder="Pesquisar por nome ou e-mail" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="w-56" />{can("users.create") && <Button size="sm" onClick={() => setEdit({ f: blank })}>Novo usuário</Button>}</>} /><CardBody>
    <DataTable rows={q.data?.items ?? []} total={q.data?.total} page={page} pageSize={20} onPage={setPage} loading={q.isLoading} onRowClick={(r) => can("users.edit") && setEdit({ id: String(r["id"]), f: { ...blank, name: String(r["name"]), email: String(r["email"]), phone: String(r["phone"] ?? ""), role_id: String(r["role_id"] ?? ""), escopos_empresas: (r["escopos_empresas"] as EscopoEmpresa[] | null) ?? [], is_active: Boolean(r["is_active"]) } })}
      columns={[{ key: "name", label: "Nome" }, { key: "email", label: "E-mail" }, { key: "phone", label: "Telefone" }, { key: "role_name", label: "Perfil", render: (r) => r["is_owner"] ? <Badge tone="violet">Proprietário</Badge> : String(r["role_name"] ?? "—") }, { key: "escopos_empresas", label: tr("termos.empresas"), render: (r) => (r["is_owner"] ? tr("acesso_empresa.modo_todas") : resumoEscopos((r["escopos_empresas"] as EscopoEmpresa[] | null) ?? [], tr)) }, { key: "is_active", label: COPY.situacao, render: (r) => <Badge tone={r["is_active"] ? "green" : "red"}>{r["is_active"] ? "Ativo" : "Inativo"}</Badge> }, { key: "last_login_at", label: "Último acesso", render: (r) => r["last_login_at"] ? dateTimeBR(r["last_login_at"] as string) : "—" }]} />
