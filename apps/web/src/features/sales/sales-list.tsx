...
export function SalesList({ kind }: { kind: string }) {
  ...
  return <DocList key={kind} title={k.title} endpoint={`/api/sales/${kind}`} base={`/vendas/${kind}`} canCreate={can(`${k.perm}.create`)} canCancel={can(`${k.perm}.delete`)}
    ...
    columns={[{ key: "code", label: "Código" }, colDate("document_date", "Data"), { key: "client_name", label: "Cliente" }, { key: "empresa_name", label: "Empresa" }, { key: "responsible_name", label: "Responsável" }, colMoney("subtotal", "Subtotal"), colMoney("discount", "Desconto"), colMoney("freight", "Frete"), colMoney("total", "Total"), colStatus()]}
