...
export function DfeApprovals() {
  ...
  return <Card><CardHeader title="Aprovação de Notas Fiscais" subtitle="Rascunhos gerados automaticamente a partir dos Perfis de Lançamento por fornecedor" actions={<Link href="/estoque?tab=entradas&sub=dfe"><Button size="sm" variant="outline">DFe Recebidas</Button></Link>} /><CardBody>
    <DataTable rows={q.data?.items ?? []} loading={q.isLoading} emptyText="Nenhuma nota aguardando aprovação." columns={[{ key: "number", label: "Nº" }, { key: "issuer_name", label: "Emitente" }, { key: "emission_date", label: "Emissão", render: (r) => dateBR(r["emission_date"] as string) }, { key: "total", label: "Total", align: "right", render: (r) => brl(r["total"] as string) }, { key: "proposed", label: "Lançar como", render: (r) => { const p = r["proposed"] as { destination: string; apportionment: unknown[] }; return `${enumLabel("invoice_destination", p.destination)} · ${p.apportionment?.length ?? 0} linha(s) de rateio`; } }]}
