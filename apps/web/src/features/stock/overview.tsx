...
import { MOVEMENT_TYPES_PT } from "./movements";
...
export function StockOverview() {
  ...
  return <div className="ws-scroll space-y-3">
    ...
    <Card><CardHeader title="Últimas movimentações do mês" actions={<Link className="text-xs text-brand-700 hover:underline" href="/estoque?tab=movimentacoes">ver ledger completo</Link>} /><CardBody><SimpleTable rows={items} cols={[{ key: "movement_date", label: "Data", render: (r) => dateBR(r["movement_date"] as string) }, { key: "movement_type", label: "Tipo", render: (r) => <Badge tone={r["direction"] === 1 ? "green" : "red"}>{MOVEMENT_TYPES_PT[String(r["movement_type"])] ?? String(r["movement_type"])}</Badge> }, { key: "product_name", label: "Produto" }, { key: "warehouse_name", label: "Armazém" }, { key: "quantity", label: "Qtd.", align: "right", render: (r) => `${r["direction"] === 1 ? "+" : "−"}${num(r["quantity"] as string, 2)}` }, { key: "total_cost", label: "Valor", align: "right", render: (r) => brl(r["total_cost"] as string) }, { key: "created_by_name", label: "Usuário" }]} /></CardBody></Card>
  </div>;
}
