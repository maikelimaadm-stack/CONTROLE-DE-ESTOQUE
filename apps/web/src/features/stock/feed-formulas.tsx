...
export function FeedFormulasPanel() {
  ...
  return <Card><CardHeader title="Formulação (Fábrica de Ração)" subtitle="Receita: matérias-primas e quantidades; vincule o produto acabado para a batida gerar estoque" actions={can("feed_formulas.create") && <Button size="sm" onClick={openNew}>Adicionar novo</Button>} /><CardBody>
    <DataTable rows={q.data?.items ?? []} loading={q.isLoading} columns={[{ key: "code", label: "Código" }, { key: "name", label: "Nome" }, { key: "product_name", label: "Produto acabado" }, { key: "total_quantity", label: "Quantidade total", align: "right", render: (r) => num(r["total_quantity"] as string, 4) }, { key: "items", label: "Matéria prima", render: (r) => ((r as Formula).items ?? []).map((i) => `${i.product_name} (${pct(i.percentage)})`).join(", ") }]}
