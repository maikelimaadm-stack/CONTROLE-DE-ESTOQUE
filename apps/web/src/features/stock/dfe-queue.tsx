...
export function DfeQueue() {
  ...
  const MAN: Record<string, string> = { none: "—", awareness: "Ciência", confirmed: "Confirmada", unknown: "Desconhecimento", not_performed: "Não realizada" }; const LAUNCH: Record<string, string> = { pending: "Pendente", draft: "Rascunho (aprovação)", launched: "Lançada", ignored: "Ignorada" };
  return <Card><CardHeader title="DFe Recebidas" subtitle="Documentos fiscais eletrônicos recebidos. A captura automática na SEFAZ depende de certificado digital (integração futura); XMLs podem ser importados manualmente." actions={<>{can("dfe.create") && <Button size="sm" onClick={() => setOpen(true)}>Buscar/Importar DFe</Button>}<Link href="/estoque?tab=entradas&sub=conferencia"><Button size="sm" variant="outline">Aprovação de Notas</Button></Link></>} /><CardBody>
    <FilterBar filters={[{ name: "search", label: "Chave/Emitente/Número", type: "text" }, { name: "launch_status", label: "Lançamento", type: "select", options: Object.entries(LAUNCH).map(([value, label]) => ({ value, label })) }, { name: "manifest_status", label: "Manifestação", type: "select", options: Object.entries(MAN).map(([value, label]) => ({ value, label })) }]} f={f} set={set} reset={() => { reset(); setApplied({}); }} onApply={() => { setApplied({ ...f }); setPage(1); }} />
    <DataTable rows={q.data?.items ?? []} total={q.data?.total} page={page} pageSize={30} onPage={setPage} loading={q.isLoading} columns={[{ key: "access_key", label: "Chave", render: (r) => <span className="font-mono text-[11px]">{String(r["access_key"])}</span> }, { key: "number", label: "Nº" }, { key: "issuer_name", label: "Emitente" }, { key: "emission_date", label: "Emissão", render: (r) => dateBR(r["emission_date"] as string) }, { key: "total", label: "Total", align: "right", render: (r) => brl(r["total"] as string) }, { key: "manifest_status", label: "Manifestação", render: (r) => MAN[String(r["manifest_status"])] }, { key: "launch_status", label: "Lançamento", render: (r) => <Badge tone={r["launch_status"] === "launched" ? "green" : r["launch_status"] === "ignored" ? "slate" : "amber"}>{LAUNCH[String(r["launch_status"])]}</Badge> }]}
