...
export function DocList({ title, endpoint, base, columns, filters, canCreate, canCancel, cancelPath, extraActions, totals, defaultFilters, rowActions, createLabel = "Novo", hideNew, entity }: { title: string; endpoint: string; base: string; columns: Column<Row>[]; filters?: Filter[]; canCreate?: boolean; canCancel?: boolean; cancelPath?: (id: string) => string; extraActions?: React.ReactNode; totals?: (t: Record<string, string>) => React.ReactNode; defaultFilters?: Record<string, string>; rowActions?: (r: Row) => { label: string; onClick?: () => void; href?: string; danger?: boolean }[]; createLabel?: string; hideNew?: boolean; /** tabela da auditoria/anexos */ entity?: string }) {
  ...
  const b1filters = React.useMemo<Base1FilterDef[]>(() => { const fs = filters ?? []; const out: Base1FilterDef[] = []; const hasRange = fs.some((x) => x.name === "start_date") && fs.some((x) => x.name === "end_date"); if (hasRange) out.push({ key: "periodo", label: "Período", kind: "date", mode: "simple", range: { from: "start_date", to: "end_date" } }); for (const x of fs) { if (hasRange && (x.name === "start_date" || x.name === "end_date")) continue; out.push({ key: x.name, label: x.label, kind: x.type === "date" ? "date" : x.type === "select" ? "enum" : x.type === "ref" ? "ref" : "text", mode: "simple", resource: x.resource, resourceFilter: x.extra, options: x.options }); }
    ...
    for (const c of columns) { if (taken.has(c.key) || c.key.startsWith("__") || takenLabels.has(String(c.label).trim().toLowerCase())) continue; out.push({ key: c.key, label: c.label, kind: c.kind ?? (c.align === "right" ? "number" : "text"), mode: "advanced", options: c.options }); }
