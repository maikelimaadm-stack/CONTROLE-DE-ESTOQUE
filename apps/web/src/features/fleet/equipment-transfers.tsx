...
/** Transferência de máquina entre empresas + histórico (ação contextual a partir da máquina: `equipmentId`). */
export function EquipmentTransfersPanel({ equipmentId }: { equipmentId?: string } = {}) {
  ...
  const [h, setH] = React.useState({ equipment_id: equipmentId ?? "", empresa_destino_id: "", transfer_date: todayISO(), note: "" });
  const create = useCreate("/api/fleet/equipment-transfers", () => { setH({ ...h, equipment_id: "" }); void q.refetch(); });
  return <div className="space-y-3">
    {can("equipment_transfers.create") && <Card><CardHeader title="Transferência de Equipamento entre Empresas" /><CardBody><div className="grid grid-cols-12 gap-3">
      ...
      <Field label="Empresa destino" required span={3}><RefSelect resource="empresas" value={h.empresa_destino_id} onChange={(v) => setH({ ...h, empresa_destino_id: v ?? "" })} /></Field>
      ...
      <div className="col-span-1 flex items-end"><Button size="sm" loading={create.isPending} disabled={!h.equipment_id || !h.empresa_destino_id} onClick={() => create.mutate({ ...h, note: h.note || null })}>Transferir</Button></div>
    </div></CardBody></Card>}
    <Card><CardHeader title="Histórico de transferências" /><CardBody>{q.isLoading ? <Spinner /> : <SimpleTable rows={q.data?.items ?? []} cols={[{ key: "transfer_date", label: "Data", render: (r) => dateBR(r["transfer_date"] as string) }, { key: "equipment_name", label: "Equipamento" }, { key: "empresa_origem_name", label: "Origem" }, { key: "empresa_destino_name", label: "Destino" }, { key: "created_by_name", label: "Usuário" }, { key: "note", label: "Observação" }]} />}</CardBody></Card>
  </div>;
}
