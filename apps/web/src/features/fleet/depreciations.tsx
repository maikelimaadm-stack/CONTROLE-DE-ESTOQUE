...
export function DepreciationsPanel() {
  ...
  return <Card><CardHeader title="Depreciações de Bens" subtitle="Cálculo mensal linear (com ou sem valor residual) por equipamento; idempotente por mês/equipamento." actions={can("depreciations.create") && <Button size="sm" loading={act.isPending} onClick={() => act.mutate({ path: "/api/assets/depreciations/run", idem: true, body: { period_month: month } })}>Calcular mês</Button>} /><CardBody>
    ...
    <DataTable rows={q.data?.items ?? []} total={q.data?.total} page={page} pageSize={50} onPage={setPage} loading={q.isLoading} columns={[{ key: "period_month", label: "Mês", render: (r) => monthBR(r["period_month"] as string) }, { key: "equipment_code", label: "Código" }, { key: "equipment_name", label: "Equipamento" }, { key: "empresa_name", label: "Empresa" }, { key: "acquisition_value", label: "Valor de aquisição", align: "right", render: (r) => brl(r["acquisition_value"] as string) }, { key: "amount", label: "Depreciação", align: "right", render: (r) => brl(r["amount"] as string) }, { key: "accumulated", label: "Acumulada", align: "right", render: (r) => brl(r["accumulated"] as string) }]} />
  </CardBody></Card>;
}
