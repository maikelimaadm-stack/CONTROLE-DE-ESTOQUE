...
const STATUS_OPTS = [["open", "A vencer"], ["overdue", "Vencida"], ["partially_paid", "Baixa parcial"], ["paid", "Baixada"], ["advance_pending", "Adiantamento/Pendente"], ["advance_paid", "Adiantamento/Baixado"], ["invoice_pending", "Fatura/Pendente"], ["invoice_paid", "Fatura/Baixado"], ["cancelled", "Cancelada"]].map(([value, label]) => ({ value: value!, label: label! }));
...
export function TitleList({ dir, initialStatus }: { dir: Dir; initialStatus?: string }) {
  ...
  const q = useQuery({ queryKey: ["titles", dir, applied, page, pageSize], queryFn: () => api<{ items: Row[]; total: number; totals: { amount: string; balance: string; paid: string }; idGlobal?: { rotulo: string } }>(`${c.endpoint}${qs({ ...applied, page, pageSize })}`) });
  const act = useAction(() => { setBatch(null); setCancelOne(null); setSel(new Set()); void qc.invalidateQueries({ queryKey: ["titles"] }); });
  /** Colunas em UMA lista: a grade e o rodapé de totais leem a mesma coisa (ver colSpanAteColuna). */
  const colunas = React.useMemo<Column<Row>[]>(() => [...(q.data?.idGlobal ? [colunaIdGlobalTabela<Row>(q.data.idGlobal.rotulo)] : []), { key: "code", label: "Código" }, { key: "number", label: "Documento" }, { key: "empresa_name", label: "Empresa" }, { key: "person_name", label: c.person }, { key: "emission_date", label: "Emissão", render: (r) => dateBR(r["emission_date"] as string) }, { key: "due_date", label: "Vencimento", render: (r) => dateBR(r["due_date"] as string) }, { key: "installment_number", label: "Parcela", render: (r) => r["installment_number"] ? `${r["installment_number"]}/${r["installment_count"] ?? "?"}` : "1/1" }, { key: "amount", label: "Valor", align: "right", render: (r) => brl(r["amount"] as string) }, { key: "balance", label: "Saldo", align: "right", render: (r) => brl(r["balance"] as string) }, { key: "status_label", label: COPY.situacao, render: (r) => <Badge tone={titleTone(String(r["status_label"]))}>{String(r["status_label"])}</Badge> }, { key: "last_settlement_date", label: "Última baixa", render: (r) => r["last_settlement_date"] ? dateBR(r["last_settlement_date"] as string) : "" }, { key: "note", label: "Observação", className: "max-w-[260px] truncate" }], [q.data?.idGlobal, c.person]);

  return <Card>
    <PageHeader inCard title={c.title} actions={<>
    ...
    <CardBody>
      ...
      <DataTable rows={q.data?.items ?? []} total={q.data?.total} page={page} pageSize={pageSize} onPage={setPage} onPageSize={setPageSize} loading={q.isLoading} selectable selected={sel} onSelect={setSel} onRowClick={(r) => router.push(`${c.base}/${r["id"]}`)}
        ...
        columns={colunas}
        footer={q.data && <tr><td colSpan={colSpanAteColuna(colunas, "amount")} className="px-2 py-1">Totais (filtro)</td><td className="num">{brl(q.data.totals.amount)}</td><td className="num">{brl(q.data.totals.balance)}</td><td colSpan={colSpanAposColuna(colunas, "balance", true)} className="px-2">Baixado: {brl(q.data.totals.paid)}</td></tr>}
...
export function TitleForm({ dir, id }: { dir: Dir; id?: string }) {
  ...
  const valid = h.empresa_id && h.number && h.person_id && Number(h.amount) > 0 && h.note && sumOk;
  ...
  return <Card><CardHeader title={`${id ? "Editar" : "Novo"} título — ${c.title}`} actions={<><Button variant="outline" size="sm" onClick={() => router.back()}>Voltar</Button><Button size="sm" loading={busy} disabled={!valid} onClick={() => (id ? upd.mutate({ path: `${c.endpoint}/${id}`, method: "PUT", body: body() }) : create.mutate(body()))}>Salvar</Button></>} /><CardBody className="space-y-4">
    ...
    <div className="grid grid-cols-12 gap-3">
      ...
      <Field label="Nº do documento" required span={2}><Input value={h.number} onChange={(e) => setH({ ...h, number: e.target.value })} /></Field>
      ...
      <Field label="Tipo de documento" span={2}><NativeSelect value={h.document_type} onChange={(e) => setH({ ...h, document_type: e.target.value })}><option value="">—</option>{DOC_TYPES.map((d) => <option key={d} value={d}>{enumLabel("document_type", d)}</option>)}</NativeSelect></Field>
      ...
      {h.payment_type === "recurring" && <><Field label="Recorrência" span={2}><NativeSelect value={h.recurrence_type} onChange={(e) => setH({ ...h, recurrence_type: e.target.value })}><option value="weekly">Semanal</option><option value="monthly">Mensal</option><option value="quarterly">Trimestral</option><option value="yearly">Anual</option></NativeSelect></Field><Field label="Quantidade de ocorrências" span={2}><Input type="number" min={1} max={60} value={h.recurrence_count} onChange={(e) => setH({ ...h, recurrence_count: e.target.value })} /></Field></>}
