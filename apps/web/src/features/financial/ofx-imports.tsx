...
export function OfxImportsPanel({ pendingOnly }: { pendingOnly?: boolean } = {}) {
  ...
  return <Card><CardHeader title="Conciliação Bancária (OFX)" subtitle="Importe o extrato OFX e concilie cada transação com um movimento existente ou crie um novo." actions={can("ofx_imports.create") && <Button size="sm" onClick={() => setOpen(true)}>Importar OFX</Button>} /><CardBody>
    {q.isLoading ? <LoadingState /> : <DataTable rows={(q.data?.items ?? []).filter((r) => !pendingOnly || r["status"] !== "reconciled")} emptyText={pendingOnly ? "Nenhuma importação pendente de conciliação." : undefined} onRowClick={(r) => router.push(`/financeiro/ofx/${r["id"]}`)} columns={[{ key: "code", label: "Código" }, { key: "description", label: "Descrição" }, { key: "bank_account_name", label: "Conta" }, { key: "start_date", label: "De", render: (r) => dateBR(r["start_date"] as string) }, { key: "end_date", label: "Até", render: (r) => dateBR(r["end_date"] as string) }, { key: "transaction_count", label: "Transações", align: "right" }, { key: "status", label: COPY.situacao, render: (r) => <StatusBadge domain="status" value={r["status"]} /> }, { key: "created_at", label: "Importado em", render: (r) => dateTimeBR(r["created_at"] as string) }]} />}
