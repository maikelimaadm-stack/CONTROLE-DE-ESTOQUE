...
export function BudgetPlanningPanel() {
  ...
  return <Card><CardHeader title="Previsão Orçamentária" subtitle="Valores previstos por natureza × mês. Cadastre o planejamento em Cadastros › Previsão Orçamentária." actions={pid && can("budget_plannings.edit") && <Button size="sm" loading={act.isPending} onClick={save}>Salvar</Button>} /><CardBody className="space-y-3">
    <Field label="Planejamento" span={4}><RefSelect resource="budget_plannings" value={pid} onChange={(v) => setPid(v ?? "")} /></Field>
    {q.isLoading && <Spinner />}
    {q.data && <div className="overflow-x-auto rounded border"><table className="table-dense w-full text-[11.5px]"><thead><tr><th className="min-w-[220px]">Natureza ({q.data.year})</th><th className="text-right">Ano anterior</th>{M.map((m) => <th key={m} className="w-24 text-right">{m}</th>)}<th className="text-right">Total</th></tr></thead><tbody>
