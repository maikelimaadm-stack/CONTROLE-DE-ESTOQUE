...
export function Base1List(props: Base1ListProps) {
  ...
  const pageSize = pageSizeLocal ?? prefs.pageSize ?? 20;
  ...
  const grand = useQuery({ queryKey: ["b1-total", moduleId, queryKeyExtra], queryFn: async () => (await fetchPage({ page: 1, pageSize: 1, filters: {} })).total, staleTime: 60_000 });
  ...
  return <div className={cn("b1 flex flex-col gap-2", props.className)} data-testid="b1-list">
    ...
    {showChips && <div className="flex items-center gap-1.5 rounded-2xl bg-white px-2 py-1.5 shadow-sm no-print">
    ...
    {!showChips && <div className="no-print"><button type="button" className="inline-flex items-center gap-1 rounded-full bg-white px-3 py-1 text-[11.5px] text-slate-500 shadow-sm hover:text-slate-800" onClick={() => setShowChips(true)}><PanelLeftOpen className="h-3.5 w-3.5" /> Exibir faixa de filtros</button></div>}
    {/* corpo */}
    <div className="flex gap-2">
      ...
      <div className="min-w-0 flex-1 rounded-2xl bg-white p-2 shadow-sm">
        ...
        {/* rodapé */}
        <div className="mt-2 flex flex-wrap items-center gap-x-6 gap-y-2 px-2 py-1.5 text-[12px] font-semibold text-slate-700 no-print">
          <span>Selecionados: {selected.size}</span><span>Listados: {rows.length}</span><span>Filtrados: {total}</span><span>Totais: {grand.data ?? total}</span>
          <span className="ml-auto flex items-center gap-2">
            <NativeSelect aria-label="Quantidade de registros por carregamento" className="h-8 w-24 rounded-full bg-slate-100 border-0 pl-3 text-[12px] font-semibold" value={pageSize} onChange={(e) => setPageSize(Number(e.target.value))}>{BASE1_PAGE_SIZES.map((n) => <option key={n} value={n}>{n}</option>)}</NativeSelect>
            <IconBtn aria-label="Carregar mais registros" title="Carregar mais registros" disabled={!q.hasNextPage || q.isFetchingNextPage} onClick={() => void q.fetchNextPage()}><ChevronsDown className="h-4 w-4" /></IconBtn>
          </span>
        </div>
      </div>
    </div>
