 ...
 */
export function Base1Grid({ columns, rows, loading, sort, onSort, selected, onSelect, onOpen, frozen = 0, onFreeze, onHide, onResize, onAutoFit, onFilter, footer, rowKey, emptyText }: {
  ...
  rowKey?: (r: Row) => string; emptyText?: string;
}) {
            ...
            <div className="flex h-[30px] items-center gap-1">
              ...
              <B1Popover className="w-52 p-1" align="start" trigger={<button type="button" aria-label={`Abrir menu da coluna ${c.label}`} className="th-menu"><MoreVertical /></button>}>
                <MenuList onPick={(k) => { if (k === "filter") onFilter?.(c.key); if (k === "fit") onAutoFit?.(c.key); if (k === "freeze") onFreeze?.(isFrozen ? i : i + 1); if (k === "hide") onHide?.(c.key); }} items={[
                  ...
                  { key: "freeze", label: isFrozen ? "Descongelar coluna" : "Congelar coluna", icon: <PanelLeft className="h-4 w-4" />, disabled: !onFreeze },
