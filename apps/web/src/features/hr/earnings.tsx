...
import { SimpleTable, useEmpresaPadrao, type Row } from "@/features/docs/shared";
import { ActionDialog, useAction } from "@/features/docs/actions";
import { COPY } from "@/lib/copy";
export function EarningsPanel() {
  const { can } = useAuth(); const empresa = useEmpresaPadrao(); const [empresaId, setEmpresaId] = React.useState(""); const [month, setMonth] = React.useState(monthStartISO()); const [sel, setSel] = React.useState<string | null>(null); const [gen, setGen] = React.useState(false);
  React.useEffect(() => { setEmpresaId((o) => o || empresa); }, [empresa]);
  ...
  return <div className="space-y-3">
    <Card><CardHeader title="Apuração Mensal (folha)" subtitle="Salário base + eventos fixos + bonificações − faltas − adiantamentos. Gera títulos a pagar por funcionário." /><CardBody>
      <div className="grid grid-cols-12 gap-2"><Field label="Empresa" span={4}><RefSelect resource="empresas" value={empresaId} onChange={(v) => setEmpresaId(v ?? "")} /></Field><Field label="Mês" span={3}><Input type="date" value={month} onChange={(e) => setMonth(e.target.value)} /></Field>{can("earnings.create") && <div className="col-span-3 flex items-end"><Button size="sm" loading={act.isPending} disabled={!empresaId} onClick={() => act.mutate({ path: "/api/hr/earnings/calculate", body: { empresa_id: empresaId, reference_month: month } })}>Calcular / recalcular</Button></div>}</div>
      {list.isLoading ? <Spinner /> : <div className="mt-3"><SimpleTable rows={list.data?.items ?? []} cols={[{ key: "reference_month", label: "Mês", render: (r) => monthBR(r["reference_month"] as string) }, { key: "code", label: "Código" }, { key: "empresa_name", label: "Empresa" }, { key: "employees", label: "Funcionários", align: "right" }, { key: "total_earnings", label: "Proventos", align: "right", render: (r) => brl(r["total_earnings"] as string) }, { key: "total_deductions", label: "Descontos", align: "right", render: (r) => brl(r["total_deductions"] as string) }, { key: "total_net", label: "Líquido", align: "right", render: (r) => brl(r["total_net"] as string) }, { key: "status", label: COPY.situacao, render: (r) => <StatusBadge domain="status" value={r["status"]} /> }, { key: "x", label: "", render: (r) => <Button size="sm" variant="outline" onClick={() => setSel(String(r["id"]))}>Detalhar</Button> }]} /></div>}
    </CardBody></Card>
    ...
    <ActionDialog open={gen} onOpenChange={setGen} title="Gerar contas a pagar da folha" loading={act.isPending} fields={[{ name: "due_date", label: "Vencimento", type: "date", required: true }, { name: "financial_category_id", label: "Natureza", type: "ref", resource: "financial_categories", required: true, filter: { kind: "analytic" } }, { name: "cost_center_id", label: "Centro de resultado", type: "ref", resource: "cost_centers", required: true, filter: { kind: "analytic" } }]} onSubmit={(v) => act.mutate({ path: `/api/hr/earnings/${sel}/generate-financial`, body: v })} />
  </div>;
}
