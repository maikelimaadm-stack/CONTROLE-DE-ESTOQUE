...
export function WeighingsList() {
  const { can } = useAuth();
  return <DocList title="Pesagens" endpoint="/api/livestock/weighings" base="/pecuaria/pesagens" canCreate={can("weighings.create")} filters={[{ name: "batch_id", label: "Lote", type: "ref", resource: "batches" }, { name: "start_date", label: "Dt. Início", type: "date" }, { name: "end_date", label: "Dt. Fim", type: "date" }]}
