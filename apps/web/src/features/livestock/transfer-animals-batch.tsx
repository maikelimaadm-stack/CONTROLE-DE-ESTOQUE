...
export function TransferAnimalsToBatch({ animalIds, onDone }: { animalIds?: string[]; onDone?: () => void } = {}) {
  const farm = useFarmDefault(); const [f, setF] = React.useState({ farm_id: "", movement_date: todayISO(), destination_batch_id: "", source_batch_id: "", note: "" }); const [sel, setSel] = React.useState<string[]>(animalIds ?? []);
