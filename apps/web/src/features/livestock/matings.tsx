...
export function MatingsPanel() {
  ...
  return <Card><CardHeader title="Acasalamentos / Coberturas" actions={can("matings.create") && <Button size="sm" onClick={() => setOpen(true)}>Nova cobertura</Button>} /><CardBody>
    ...
    <DataTable rows={q.data?.items ?? []} total={q.data?.total} page={page} pageSize={50} onPage={setPage} loading={q.isLoading} columns={[{ key: "mating_date", label: "Data", render: (r) => dateBR(r["mating_date"] as string) }, { key: "season_name", label: "Estação" }, { key: "dam_identification", label: "Matriz" }, { key: "sire_name", label: "Touro / sêmen" }, { key: "mating_type", label: "Tipo", render: (r) => enumLabel("mating_type", r["mating_type"]) }, { key: "inseminator_name", label: "Inseminador" }, { key: "result", label: "Diagnóstico", render: (r) => <Badge tone={r["result"] === "pregnant" ? "green" : r["result"] === "empty" ? "red" : "amber"}>{enumLabel("diagnosis_result", r["result"])}</Badge> }, { key: "expected_birth_date", label: "Parto previsto", render: (r) => r["expected_birth_date"] ? dateBR(r["expected_birth_date"] as string) : "" }, { key: "x", label: "", render: (r) => r["result"] === "pending" && can("pregnancy_diagnosis.create") ? <Button size="sm" variant="outline" onClick={() => setDiag(String(r["id"]))}>Diagnóstico</Button> : null }]} />
    <Dialog open={open} onOpenChange={setOpen} title="Nova cobertura (uma por matriz selecionada)" size="xl" footer={<Button size="sm" loading={create.isPending} disabled={!f.season_id || !dams.length} onClick={() => create.mutate({ ...f, dam_ids: dams, sire_id: f.sire_id || null, protocol_id: f.protocol_id || null, inseminator_person_id: f.inseminator_person_id || null, note: f.note || null })}>Salvar</Button>}>
      <div className="mb-2 grid grid-cols-12 gap-2"><Field label="Estação" required span={4}><RefSelect resource="breeding_seasons" value={f.season_id} onChange={(v) => setF({ ...f, season_id: v ?? "" })} /></Field><Field label="Data" span={2}><Input type="date" value={f.mating_date} onChange={(e) => setF({ ...f, mating_date: e.target.value })} /></Field><Field label="Tipo" span={2}><NativeSelect value={f.mating_type} onChange={(e) => setF({ ...f, mating_type: e.target.value })}>{enumOptions("mating_type").map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</NativeSelect></Field><Field label="Touro / sêmen" span={4}><RefSelect resource="breeding_sires" value={f.sire_id} onChange={(v) => setF({ ...f, sire_id: v ?? "" })} /></Field><Field label="Protocolo" span={4}><RefSelect resource="breeding_protocols" value={f.protocol_id} onChange={(v) => setF({ ...f, protocol_id: v ?? "" })} /></Field><Field label="Inseminador" span={4}><RefSelect resource="people" value={f.inseminator_person_id} onChange={(v) => setF({ ...f, inseminator_person_id: v ?? "" })} /></Field><Field label="Observação" span={4}><Input value={f.note} onChange={(e) => setF({ ...f, note: e.target.value })} /></Field></div>
