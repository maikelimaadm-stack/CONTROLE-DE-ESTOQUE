...
export function GroupBatches({ sourceBatchIds, onDone }: { sourceBatchIds?: string[]; onDone?: () => void } = {}) {
  const empresa = useEmpresaPadrao(); const [f, setF] = React.useState({ empresa_id: "", movement_date: todayISO(), destination_batch_id: "", close_sources: "true" }); const [src, setSrc] = React.useState<string[]>(sourceBatchIds?.length ? sourceBatchIds : [""]);
