...
export function TransferBatchLocation({ batchId, onDone, history = true }: { batchId?: string; onDone?: () => void; history?: boolean } = {}) {
  const farm = useFarmDefault(); const [f, setF] = React.useState({ farm_id: "", movement_date: todayISO(), batch_id: batchId ?? "", grazing_module_id: "", area_id: "", corral_id: "", note: "" });
  ...
  {history && <DocList title="Histórico" endpoint="/api/livestock/movements" base="/pecuaria/movimentacoes/module_area_transfer" defaultFilters={{ movement_type: "module_area_transfer" }} hideNew columns={[{ key: "code", label: "Código" }, colDate("movement_date", "Data"), { key: "batch_name", label: "Lote" }, { key: "note", label: "Destino / observação" }]} />}</div>;
}
