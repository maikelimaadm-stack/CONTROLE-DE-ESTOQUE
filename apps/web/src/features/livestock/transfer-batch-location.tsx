...
export function TransferBatchLocation({ batchId, onDone, history = true }: { batchId?: string; onDone?: () => void; history?: boolean } = {}) {
  ...
  {history && <DocList title="Histórico" endpoint="/api/livestock/movements" base="/pecuaria/movimentacoes/module_area_transfer" defaultFilters={{ movement_type: "module_area_transfer" }} hideNew columns={[{ key: "code", label: "Código" }, colDate("movement_date", "Data"), { key: "batch_name", label: "Lote" }, { key: "note", label: "Destino / obs." }]} />}</div>;
}
