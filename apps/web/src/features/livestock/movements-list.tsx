...
export function LivestockMovementsList({ type }: { type: string }) {
  ...
  return <DocList key={type} title={`Rebanho — ${MOV_PT[type] ?? type}`} endpoint="/api/livestock/movements" base={`/pecuaria/movimentacoes/${type}`} defaultFilters={{ movement_type: type }} canCreate={can(`${perm}.create`)} canCancel={can(`${perm}.delete`)}
