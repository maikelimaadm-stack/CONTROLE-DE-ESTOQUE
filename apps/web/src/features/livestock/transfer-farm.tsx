...
export function TransferToFarm({ batchId, animalIds, onDone, history = true }: { batchId?: string; animalIds?: string[]; onDone?: () => void; history?: boolean } = {}) {
  const farm = useFarmDefault(); const [f, setF] = React.useState({ farm_id: "", destination_farm_id: "", movement_date: todayISO(), batch_id: batchId ?? "", destination_batch_id: "", note: "", mode: animalIds?.length ? "animals" : "batch" }); const [sel, setSel] = React.useState<string[]>(animalIds ?? []);
  React.useEffect(() => { setF((o) => ({ ...o, farm_id: o.farm_id || farm })); }, [farm]);
  const create = useCreate("/api/livestock/transfers/to-farm", () => { setSel([]); onDone?.(); });
