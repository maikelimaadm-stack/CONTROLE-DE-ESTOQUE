...
export function AnimalsList() {
  ...
  const q = useQuery({ queryKey: ["animals", applied, page, pageSize], queryFn: () => api<{ items: Row[]; total: number; totals: { weight: string; value: string }; idGlobal?: { rotulo: string } }>(`/api/livestock/animals${qs({ ...applied, page, pageSize })}`) });
  const rowMenu = (r: Row) => [
