 ...
 * e as ações contextuais por animal (mover para lote, transferir de empresa).
 */
export function AnimalsList() {
  ...
  const rowMenu = (r: Row) => [
    ...
    ...(can("batch_farm_transfer.create") ? [{ label: "Transferir de empresa", onClick: () => herd.open("fazendas", { animalIds: [String(r["id"])] }) }] : []),
  ...
  return <><Card><CardHeader title="Animais (rebanho individualizado)" actions={<>
    ...
    <FilterBar f={f} set={set} reset={() => { reset(); setApplied({}); }} onApply={() => { setApplied({ ...f }); setPage(1); }} filters={[{ name: "search", label: "Identificação (brinco, SISBOV, chip, nome)", type: "text" }, { name: "batch_id", label: "Lote", type: "ref", resource: "batches" }, { name: "category_id", label: "Categoria", type: "ref", resource: "animal_categories" }, { name: "breed_id", label: "Raça", type: "ref", resource: "breeds" }, { name: "sex", label: "Sexo", type: "select", options: [{ value: "M", label: "Macho" }, { value: "F", label: "Fêmea" }] }, { name: "status", label: COPY.situacao, type: "select", options: enumOptions("status", ["active", "sold", "dead", "lost", "transferred"]) }, { name: "empresa_id", label: "Empresa", type: "ref", resource: "farms" }]} />
