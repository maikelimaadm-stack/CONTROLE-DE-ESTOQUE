...
import { Button, Card, CardHeader, CardBody, Input, StatusBadge } from "@/components/ui";
...
export function LocateAnimalPanel() {
  ...
  return <Card><CardHeader title="Localizar Animal" subtitle="Busca por qualquer identificação (brinco, SISBOV, chip) e mostra empresa, lote, módulo e área atuais" /><CardBody className="space-y-3">
    ...
    <SimpleTable rows={q.data?.items ?? []} cols={[{ key: "identification", label: "Identificação", render: (r) => <Link className="text-brand-700 underline" href={`/pecuaria/animais/${r["id"]}`}>{String(r["identification"])}</Link> }, { key: "identification_type", label: "Tipo" }, { key: "category_name", label: "Categoria" }, { key: "empresa_name", label: "Empresa" }, { key: "batch_name", label: "Lote" }, { key: "module_name", label: "Módulo" }, { key: "area_name", label: "Área" }, { key: "status", label: COPY.situacao, render: (r) => <StatusBadge domain="status" value={r["status"]} /> }]} />
  </CardBody></Card>;
}
