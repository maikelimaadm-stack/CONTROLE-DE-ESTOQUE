...
import { NAV, flatNav, navKey, hrefKey, permOk, type NavItem } from "@/lib/nav";
 ...
 */
function NavNode({ item, depth, can, pathname, open, toggle, onNavigate }: { item: NavItem; depth: number; can: (p: string) => boolean; /** chave da rota atual (caminho + aba) */ pathname: string; open: Set<string>; toggle: (k: string) => void; onNavigate: () => void }) {
  const visible = (i: NavItem): boolean => i.href ? permOk(can, i.perm) : (i.children ?? []).some(visible);
  if (!visible(item)) return null;
  const key = item.label + depth;
  if (item.href) { const active = pathname === hrefKey(item.href); return <Link href={item.href} onClick={onNavigate} className={cn("mg-ni", active && "active")} style={{ paddingLeft: 12 + depth * 10 }}><FolderOpen /><span className="truncate">{item.label}</span></Link>; }
...
function ShellInner({ children }: { children: React.ReactNode }) {
  ...
  const sp = useSearchParams(); const current = navKey(pathname, sp); const currentRoute = sp.get("tab") ? `${pathname}?tab=${sp.get("tab")}` : pathname;
  ...
  const crumbs = React.useMemo(() => (flat.find((f) => hrefKey(f.href) === current) ?? flat.find((f) => f.href.split("?")[0] === pathname))?.path ?? [], [flat, current, pathname]);
  const results = search.length > 1 ? flat.filter((f) => permOk(can, f.perm) && f.label.toLowerCase().includes(search.toLowerCase())).slice(0, 10) : [];
  ...
  const isFav = ctx?.favorites.some((f) => f.route === currentRoute);
  const toggleFav = () => { if (!ctx) return; const label = crumbs[crumbs.length - 1] ?? pathname; const favs = isFav ? ctx.favorites.filter((f) => f.route !== currentRoute) : [...ctx.favorites, { route: currentRoute, label }]; favMut.mutate(favs); };
          ...
          <div className="relative inline-flex items-center" onMouseEnter={() => { if (!pinned) setHover(true); }} onMouseLeave={() => { if (!pinned) setHover(false); }}>
            ...
            <nav className={cn("mg-sidebar", menuOpen && "is-open")} aria-label="Menu principal">
              ...
              <div className="min-h-0 flex-1 space-y-0.5 overflow-y-auto px-2 py-2">
                ...
                {ctx.favorites.length > 0 && <div><div className="mg-ng">Favoritos</div>{ctx.favorites.map((f) => <Link key={f.route} href={f.route} onClick={closeIfFloating} className={cn("mg-ni", hrefKey(f.route) === current && "active")}><Star className="text-amber-400" /><span className="truncate">{f.label}</span></Link>)}</div>}
                {NAV.filter((i) => i.href !== "/").map((i) => <NavNode key={i.label} item={i} depth={0} can={can} pathname={current} open={open} toggle={toggle} onNavigate={closeIfFloating} />)}
              </div>
