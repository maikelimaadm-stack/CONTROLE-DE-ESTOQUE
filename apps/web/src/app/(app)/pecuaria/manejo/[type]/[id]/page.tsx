...
export default function Page({ params }: { params: Promise<{ type: string; id: string }> }) {
  ...
  return <LoadingOr q={q}>{d && <DetailShell title={`${HANDLING_PT[kind] ?? "Manejo"} ${String(d["code"])}`} subtitle={`${String(d["empresa_name"])} · ${dateBR(d["handling_date"] as string)}`} backHref={`/pecuaria?tab=manejos&type=${kind}`} breadcrumbs={[{ label: "Pecuária", href: "/pecuaria" }, { label: "Manejos", href: `/pecuaria?tab=manejos&type=${kind}` }, { label: String(d["code"]) }]}>
    <KV items={[["Empresa", String(d["empresa_name"])], ["Data", dateBR(d["handling_date"] as string)], ["Lote", String(d["batch_name"] ?? "—")], ["Produto", String(d["product_name"] ?? "—")], ["Armazém", String(d["warehouse_name"] ?? "—")], ["Quantidade total", d["quantity"] ? num(d["quantity"] as string, 4) : "—"], ["Custo", d["total"] ? brl(d["total"] as string) : "—"], ["Carência até", d["withdrawal_until"] ? dateBR(d["withdrawal_until"] as string) : "—"], ["Animais", String(d["animals_count"] ?? "")], ["Responsável", String(d["responsible"] ?? "—")], ["Registrado por", String(d["created_by_name"] ?? "—")], ["Observação", String(d["note"] ?? "—")]]} />
