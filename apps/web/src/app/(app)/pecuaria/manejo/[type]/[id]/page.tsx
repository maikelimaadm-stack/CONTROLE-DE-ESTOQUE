...
export default function Page({ params }: { params: Promise<{ type: string; id: string }> }) {
  ...
  return <LoadingOr q={q}>{d && <DetailShell title={`${HANDLING_PT[kind] ?? "Manejo"} ${String(d["code"])}`} back={`/pecuaria?tab=manejos&type=${kind}`}>
    ...
    <SimpleTable rows={d.items} cols={[{ key: "identifications", label: "Animal / lote", render: (r) => String(r["identifications"] ?? (r["herd_lot_category"] ? `Lote por contagem — ${r["herd_lot_category"]}` : "(por contagem)")) }, { key: "category_name", label: "Categoria" }, { key: "quantity", label: "Qtd.", align: "right", render: (r) => num(String(r["quantity"] ?? "0"), 4) }, { key: "dose", label: "Dose", align: "right", render: (r) => r["dose"] ? num(r["dose"] as string, 4) : "" }, { key: "new_batch_name", label: "Novo lote" }, { key: "new_category_name", label: "Nova categoria" }]} />
