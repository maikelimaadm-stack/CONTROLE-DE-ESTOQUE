...
export default function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <LoadingOr q={q}>{d && <DetailShell title={`Pesagem ${String(d["code"])}`} subtitle={`${String(d["empresa_name"])} · ${dateBR(d["weighing_date"] as string)}`} backHref="/pecuaria?tab=manejos&type=weighing" breadcrumbs={[{ label: "Pecuária", href: "/pecuaria" }, { label: "Pesagens", href: "/pecuaria?tab=manejos&type=weighing" }, { label: String(d["code"]) }]}>
    <KV items={[["Empresa", String(d["empresa_name"])], ["Data", dateBR(d["weighing_date"] as string)], ["Lote", String(d["batch_name"] ?? "—")], ["Animais", String(d["animals_count"] ?? "")], ["Peso total", `${num(String(d["total_weight"] ?? "0"), 1)} kg`], ["Responsável", String(d["responsible"] ?? "—")], ["Registrado por", String(d["created_by_name"] ?? "—")], ["Observação", String(d["note"] ?? "—")]]} />
