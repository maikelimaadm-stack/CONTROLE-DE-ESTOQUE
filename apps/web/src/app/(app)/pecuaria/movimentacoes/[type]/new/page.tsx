...
export default function Page({ params }: { params: Promise<{ type: string }> }) {
  const { type } = use(params); const router = useRouter(); const empresa = useEmpresaPadrao();
  const isIn = type === "purchase" || type === "birth"; const money = type === "purchase" || type === "sale";
  const [h, setH] = React.useState({ empresa_id: "", movement_date: todayISO(), person_id: "", batch_id: "", cause: "", note: "", invoice_number: "", generate_financial: false, due_date: "", financial_category_id: "", cost_center_id: "" });
  ...
  const valid = h.empresa_id && (mode === "animals" && !isIn ? sel.length > 0 : items.length > 0 && items.every((it) => (isIn ? it.category_id : it.herd_lot_id)));
  return <Card><CardHeader title={`Nova ${MOV_PT[type] ?? type}`} subtitle={isIn ? "Entrada de animais: por contagem (lote) com opção de identificações individuais. Compras podem gerar contas a pagar." : "Saída de animais individualizados (seleção) ou por contagem (lote). Vendas podem gerar contas a receber."} actions={<><Button variant="outline" size="sm" onClick={() => router.back()}>Voltar</Button><Button size="sm" loading={create.isPending} disabled={!valid} onClick={() => create.mutate(body())}>Salvar</Button></>} /><CardBody className="space-y-4">
    ...
    {mode === "animals" && !isIn ? <>
      <AnimalPicker selected={sel} onChange={(ids, rows) => { setSel(ids); setSelRows(rows); }} empresaId={h.empresa_id || undefined} />
      ...
      {items.map((it, i) => <div key={i} className="grid grid-cols-12 gap-2 rounded border p-2">
        {isIn ? <><Field label="Categoria" required span={3}><RefSelect resource="animal_categories" value={it.category_id} onChange={(v) => upd(i, { category_id: v ?? "" })} /></Field><Field label="Raça" span={2}><RefSelect resource="breeds" value={it.breed_id} onChange={(v) => upd(i, { breed_id: v ?? "" })} /></Field><Field label="Sexo" span={1}><NativeSelect value={it.sex} onChange={(e) => upd(i, { sex: e.target.value })}><option value="">—</option><option value="M">Macho</option><option value="F">Fêmea</option></NativeSelect></Field></>
          : <Field label="Lote (contagem)" required span={6}><HerdLotSelect value={it.herd_lot_id} onChange={(v, r) => upd(i, { herd_lot_id: v, category_id: String(r?.["category_id"] ?? "") })} empresaId={h.empresa_id || undefined} /></Field>}
        ...
        {money && <Field label="Valor unitário" span={2}><Input type="number" step="0.01" value={it.unit_value} onChange={(e) => upd(i, { unit_value: e.target.value })} /></Field>}
    ...
    {money && <div className="grid grid-cols-12 gap-3 rounded border bg-slate-50 p-3">
      ...
      {h.generate_financial && <><Field label="Vencimento" span={2}><Input type="date" value={h.due_date} onChange={(e) => setH({ ...h, due_date: e.target.value })} /></Field><Field label="Natureza" span={4}><RefSelect resource="financial_categories" value={h.financial_category_id} onChange={(v) => setH({ ...h, financial_category_id: v ?? "" })} filter={{ kind: "analytic" }} /></Field><Field label="Centro de resultado" span={3}><RefSelect resource="cost_centers" value={h.cost_center_id} onChange={(v) => setH({ ...h, cost_center_id: v ?? "" })} filter={{ kind: "analytic" }} /></Field></>}
    </div>}
  </CardBody></Card>;
}
