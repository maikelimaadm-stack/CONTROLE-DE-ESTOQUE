...
export default function Page({ params }: { params: Promise<{ type: string; id: string }> }) {
  ...
  return <LoadingOr q={q}>{d && <DetailShell title={`${MOV_PT[String(d["movement_type"])] ?? type} ${String(d["code"])}`} back={`/pecuaria/movimentacoes/${type}`} status={String(d["status"])} actions={<>
    ...
    {d["status"] !== "cancelled" && can("animal_sales.delete") && <Button size="sm" variant="danger" onClick={() => setC(true)}>Cancelar movimentação</Button>}</>}>
    <KV items={[["Empresa", String(d["empresa_name"])], ["Data", dateBR(d["movement_date"] as string)], ["Pessoa", String(d["person_name"] ?? "—")], ["Lote", String(d["batch_name"] ?? "—")], ["Cabeças", String(d["quantity"] ?? "")], ["Peso total", d["total_weight"] ? `${num(d["total_weight"] as string, 1)} kg` : "—"], ["Valor", d["total_value"] ? brl(d["total_value"] as string) : "—"], ["NF", String(d["invoice_number"] ?? "—")], ["Causa", String(d["cause"] ?? "—")], ["Financeiro", d["title_id"] ? "Gerado" : "Não gerado"], ["Observação", String(d["note"] ?? "")]]} />
    <SimpleTable rows={d.items} cols={[{ key: "identifications", label: "Identificação", render: (r) => String(r["identifications"] ?? "(por contagem)") }, { key: "category_name", label: "Categoria" }, { key: "sex", label: "Sexo", render: (r) => r["sex"] ? enumLabel("sex", r["sex"]) : "" }, { key: "quantity", label: "Cabeças", align: "right" }, { key: "weight", label: "Peso", align: "right", render: (r) => r["weight"] ? num(r["weight"] as string, 1) : "" }, { key: "unit_value", label: "Valor unitário", align: "right", render: (r) => r["unit_value"] ? brl(r["unit_value"] as string) : "" }]} />
    ...
    <Confirm open={proc} onOpenChange={setProc} title="Processar transferência" text="Os animais passam para a empresa/lote de destino." loading={act.isPending} onConfirm={() => act.mutate({ path: `/api/livestock/transfers/${id}/process`, body: {} })} />
