...
export default function Page() {
  ...
  return <Card><CardHeader title="Novo documento fiscal de entrada" actions={<><Button variant="outline" size="sm" onClick={() => router.back()}>Voltar</Button><Button size="sm" loading={create.isPending} onClick={submit} disabled={!items.length}>Salvar</Button></>} /><CardBody className="space-y-4">
    ...
    <div className="grid grid-cols-12 gap-3">
      ...
      <Field label="Data de emissão" required span={2}><Input type="date" value={h.emission_date} onChange={(e) => setH({ ...h, emission_date: e.target.value })} /></Field>
      <Field label="Data de entrega" span={2}><Input type="date" value={h.delivery_date} onChange={(e) => setH({ ...h, delivery_date: e.target.value })} /></Field>
      <Field label="UF" span={1}><Input maxLength={2} value={h.state_code} onChange={(e) => setH({ ...h, state_code: e.target.value.toUpperCase() })} /></Field>
      <Field label="Tipo de nota" required span={2}><NativeSelect value={h.document_type} onChange={(e) => setH({ ...h, document_type: e.target.value })}>{["nfe", "cte", "nfse", "nfce", "danfe", "darf", "dare", "gru", "other"].map((t) => <option key={t} value={t}>{enumLabel("document_type", t)}</option>)}</NativeSelect></Field>
      ...
      <Field label="Tipo de rateio" required span={2}><NativeSelect value={h.apportionment_type} onChange={(e) => setH({ ...h, apportionment_type: e.target.value })}><option value="by_product">Por produto</option><option value="by_value">Por valor</option></NativeSelect></Field>
