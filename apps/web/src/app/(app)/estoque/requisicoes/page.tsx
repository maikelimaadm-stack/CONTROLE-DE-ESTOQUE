...
export default function Page() {
  ...
  return <DocList title="Requisição do Estoque" endpoint="/api/stock/requisitions" base="/estoque/requisicoes" canCreate={can("requisitions.create")} canCancel={can("requisitions.delete")} filters={[{ name: "product_id", label: "Produto", type: "ref", resource: "products" }, { name: "warehouse_id", label: "Armazém", type: "ref", resource: "warehouses" }, { name: "cost_center_id", label: "Centro de Custo", type: "ref", resource: "cost_centers" }, ...dateFilters]} columns={[colText("code", "Código"), colDate("requisition_date", "Dt. Emissão"), colText("requester_name", "Solicitante"), colText("created_by_name", "Usuário"), { key: "items_summary", label: "Itens", render: (r) => <span className="max-w-[320px] truncate inline-block" title={String(r["items_summary"] ?? "")}>{String(r["items_summary"] ?? "")}</span> }, colMoney("total_amount", "Valor"), { key: "signature_status", label: "Assinatura", render: (r) => <StatusBadge s={String(r["signature_status"])} /> }, colStatus()]}
