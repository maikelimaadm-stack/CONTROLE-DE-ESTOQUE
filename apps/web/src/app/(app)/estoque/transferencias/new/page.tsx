...
function Inner() {
  ...
  const [h, setH] = React.useState({ kind: sp.get("kind") === "empresa" ? "empresa" : "warehouse", transfer_date: todayISO(), empresa_origem_id: "", origin_warehouse_id: "", empresa_destino_id: "", destination_warehouse_id: "", harvest_id: "", generate_financial: "false", proprietary_id: "", is_deductible: "false" });
  ...
  React.useEffect(() => { setH((o) => ({ ...o, empresa_origem_id: o.empresa_origem_id || empresa })); }, [empresa]);
  const create = useCreate("/api/stock/transfers", () => router.push(`/estoque?tab=transferencias&sub=${h.kind}`));
  const isFarm = h.kind === "empresa";
  const submit = () => create.mutate({ kind: h.kind, transfer_date: h.transfer_date, empresa_origem_id: h.empresa_origem_id, origin_warehouse_id: h.origin_warehouse_id, empresa_destino_id: isFarm ? h.empresa_destino_id : undefined, destination_warehouse_id: h.destination_warehouse_id, harvest_id: h.harvest_id || null, generate_financial: isFarm && h.generate_financial === "true", proprietary_id: h.proprietary_id || null, is_deductible: h.is_deductible === "true", plan: plan.installments > 1 || plan.has_down_payment ? plan : null, income_apportionment: inc.length ? toAppLines(inc) : undefined, expense_apportionment: exp.length ? toAppLines(exp) : undefined, items: items.map((i) => ({ product_id: i.product_id, quantity: i.quantity, provider_lot: i.provider_lot || null, cost_center_id: i.cost_center_id || null })) });
  return <Card><CardHeader title={isFarm ? "Transferência de Armazém entre Empresas" : "Transferência de Armazém"} actions={<><Button variant="outline" size="sm" onClick={() => router.back()}>Voltar</Button><Button size="sm" loading={create.isPending} disabled={!items.length} onClick={submit}>Salvar</Button></>} /><CardBody className="space-y-4">
    <div className="grid grid-cols-12 gap-3">
      <Field label="Tipo de movimentação" span={2}><NativeSelect value={h.kind} onChange={(e) => setH({ ...h, kind: e.target.value })}><option value="warehouse">Entre armazéns</option><option value="empresa">Entre empresas</option></NativeSelect></Field>
      ...
      <Field label="Empresa" required span={4}><RefSelect resource="empresas" value={h.empresa_origem_id} onChange={(v) => setH({ ...h, empresa_origem_id: v ?? "", origin_warehouse_id: "" })} /></Field>
      <Field label="Armazém" required span={4}><RefSelect resource="warehouses" value={h.origin_warehouse_id} onChange={(v) => setH({ ...h, origin_warehouse_id: v ?? "" })} filter={{ empresa_id: h.empresa_origem_id }} /></Field>
      {isFarm && <Field label="Empresa destino" required span={4}><RefSelect resource="empresas" value={h.empresa_destino_id} onChange={(v) => setH({ ...h, empresa_destino_id: v ?? "", destination_warehouse_id: "" })} /></Field>}
      <Field label="Armazém destino" required span={4}><RefSelect resource="warehouses" value={h.destination_warehouse_id} onChange={(v) => setH({ ...h, destination_warehouse_id: v ?? "" })} filter={{ empresa_id: isFarm ? h.empresa_destino_id : h.empresa_origem_id }} /></Field>
