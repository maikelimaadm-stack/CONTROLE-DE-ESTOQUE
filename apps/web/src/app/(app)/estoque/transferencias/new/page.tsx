...
function Inner() {
  ...
  const create = useCreate("/api/stock/transfers", () => router.push(`/estoque/transferencias?kind=${h.kind}`));
  ...
  return <Card><CardHeader title={isFarm ? "Transferência de Armazém entre Fazendas" : "Transferência de Armazém"} actions={<><Button variant="outline" size="sm" onClick={() => router.back()}>Voltar</Button><Button size="sm" loading={create.isPending} disabled={!items.length} onClick={submit}>Salvar</Button></>} /><CardBody className="space-y-4">
    <div className="grid grid-cols-12 gap-3">
      <Field label="Tipo Movimentação" span={2}><NativeSelect value={h.kind} onChange={(e) => setH({ ...h, kind: e.target.value })}><option value="warehouse">Entre armazéns</option><option value="farm">Entre fazendas</option></NativeSelect></Field>
      <Field label="Dt. Criação" required span={2}><Input type="date" value={h.transfer_date} onChange={(e) => setH({ ...h, transfer_date: e.target.value })} /></Field>
    ...
    {isFarm && h.generate_financial === "true" && <Tabs tabs={[{ value: "plan", label: "Condição de Pagamento - Parcelamento", content: <PlanEditor plan={plan} onChange={setPlan} /> }, { value: "inc", label: "Rateio - Armazém Origem / A Receber", content: <ApportionmentEditor lines={inc} onChange={setInc} total={0} /> }, { value: "exp", label: "Rateio - Armazém Destino / A Pagar", content: <ApportionmentEditor lines={exp} onChange={setExp} total={0} /> }]} />}
