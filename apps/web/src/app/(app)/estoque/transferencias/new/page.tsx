...
function Inner() {
  ...
  const [h, setH] = React.useState({ kind: sp.get("kind") === "farm" ? "farm" : "warehouse", transfer_date: todayISO(), empresa_origem_id: "", origin_warehouse_id: "", empresa_destino_id: "", destination_warehouse_id: "", harvest_id: "", generate_financial: "false", proprietary_id: "", is_deductible: "false" });
  ...
  return <Card><CardHeader title={isFarm ? "Transferência de Armazém entre Empresas" : "Transferência de Armazém"} actions={<><Button variant="outline" size="sm" onClick={() => router.back()}>Voltar</Button><Button size="sm" loading={create.isPending} disabled={!items.length} onClick={submit}>Salvar</Button></>} /><CardBody className="space-y-4">
    <div className="grid grid-cols-12 gap-3">
      <Field label="Tipo de movimentação" span={2}><NativeSelect value={h.kind} onChange={(e) => setH({ ...h, kind: e.target.value })}><option value="warehouse">Entre armazéns</option><option value="farm">Entre empresas</option></NativeSelect></Field>
