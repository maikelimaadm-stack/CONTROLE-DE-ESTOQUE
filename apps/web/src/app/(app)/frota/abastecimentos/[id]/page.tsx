...
export default function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <LoadingOr q={q}>{d && <DetailShell title={`Abastecimento ${String(d["code"])}`} back="/frota?tab=abastecimentos" status={String(d["status"])} actions={d["status"] !== "cancelled" && can("fuel_supplies.delete") && <Button size="sm" variant="danger" onClick={() => setC(true)}>Cancelar (estorna estoque)</Button>}>
    <KV items={[["Fazenda", String(d["farm_name"])], ["Data", dateBR(d["supply_date"] as string)], ["Máquina", `${d["equipment_code"] ?? ""} ${d["equipment_name"] ?? ""}`.trim()], ["Combustível", String(d["product_name"])], ["Armazém / tanque", String(d["warehouse_name"] ?? "—")], ["Operador", String(d["operator_name"] ?? "—")], ["Litros", num(String(d["quantity"] ?? "0"), 3)], ["Vl. unitário", brl(String(d["unit_value"] ?? "0"))], ["Total", brl(String(d["total"] ?? "0"))], ["Horímetro", d["hour_meter"] ? num(d["hour_meter"] as string, 2) : "—"], ["Km", d["mileage"] ? num(d["mileage"] as string, 2) : "—"], ["Centro de custo", String(d["cost_center_name"] ?? "—")], ["Safra", String(d["harvest_name"] ?? "—")], ["Origem", String(d["origin"] ?? "manual")], ["Observação", String(d["note"] ?? "—")]]} />
