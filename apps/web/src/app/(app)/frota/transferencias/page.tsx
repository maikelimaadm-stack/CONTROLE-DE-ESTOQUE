...
export default function Page() {
  ...
  const [h, setH] = React.useState({ equipment_id: "", destination_farm_id: "", transfer_date: todayISO(), note: "" });
  const create = useCreate("/api/fleet/equipment-transfers", () => { setH({ ...h, equipment_id: "" }); void q.refetch(); });
  return <div className="space-y-3">
    {can("equipment_transfers.create") && <Card><CardHeader title="Transferência de Máquina entre Fazendas" /><CardBody><div className="grid grid-cols-12 gap-3">
    ...
    <Card><CardHeader title="Histórico de transferências" /><CardBody>{q.isLoading ? <Spinner /> : <SimpleTable rows={q.data?.items ?? []} cols={[{ key: "transfer_date", label: "Data", render: (r) => dateBR(r["transfer_date"] as string) }, { key: "equipment_name", label: "Máquina" }, { key: "origin_farm_name", label: "Origem" }, { key: "destination_farm_name", label: "Destino" }, { key: "created_by_name", label: "Usuário" }, { key: "note", label: "Observação" }]} />}</CardBody></Card>
  </div>;
}
