...
const STAGES: Record<string, { title: string; hint: string }> = {
...
export default function Page({ params }: { params: Promise<{ stage: string }> }) {
  const { stage } = use(params); const { can } = useAuth(); const st = STAGES[stage] ?? STAGES["request"]!;
  const act = useAction();
  return <DocList key={stage} title={`Suprimentos — ${st.title}`} endpoint="/api/supply/requests" base="/suprimentos/view" defaultFilters={{ stage }} canCreate={can("purchase_requests.create")} hideNew={stage !== "request" && stage !== "mine"}
    extraActions={<><span className="text-xs text-slate-500">{st.hint}</span><Link href="/suprimentos/new"><Button size="sm" variant="outline">Nova Solicitação</Button></Link></>}
