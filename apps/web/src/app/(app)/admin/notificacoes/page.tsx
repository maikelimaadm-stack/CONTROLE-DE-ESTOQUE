...
export default function Page() {
  ...
  return <Card><CardHeader title="Notificações" subtitle="Geradas a partir de regras do sistema (compras paradas, estoque mínimo, vencimentos, aniversários, documentos)." actions={<><Button size="sm" variant="outline" loading={act.isPending} onClick={() => act.mutate({ path: "/api/admin/notifications/refresh" })}>Atualizar alertas</Button><Button size="sm" variant="outline" onClick={() => act.mutate({ path: "/api/admin/notifications/read-all" })}>Marcar todas como lidas</Button></>} /><CardBody>
    {q.isLoading ? <Spinner /> : <ul className="divide-y">{q.data?.items.map((n) => <li key={String(n["id"])} className={`flex items-center gap-3 py-2 text-sm ${n["read_at"] ? "text-slate-400" : ""}`}><Badge tone={n["read_at"] ? "slate" : "amber"}>{enumLabel("notification_kind", n["kind"])}</Badge><span className="flex-1">{n["route"] ? <Link className="hover:underline" href={String(n["route"])} onClick={() => act.mutate({ path: `/api/admin/notifications/${n["id"]}/read` })}>{String(n["title"])}</Link> : String(n["title"])}</span><span className="text-xs">{dateTimeBR(n["created_at"] as string)}</span>{!n["read_at"] && <Button size="sm" variant="ghost" onClick={() => act.mutate({ path: `/api/admin/notifications/${n["id"]}/read` })}>Lida</Button>}</li>)}{q.data?.items.length === 0 && <li className="py-6 text-center text-slate-400">Nenhuma notificação.</li>}</ul>}
  </CardBody></Card>;
}
