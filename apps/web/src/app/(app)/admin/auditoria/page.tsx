...
import { Badge, Card, CardHeader, CardBody, Dialog } from "@/components/ui";
...
export default function Page() {
  ...
  return <Card><CardHeader title="Auditoria" subtitle="Trilha imutável de criações, alterações, cancelamentos, logins e ações de fluxo (antes/depois em JSON)." /><CardBody>
    <FilterBar f={f} set={set} reset={() => { reset(); setApplied({}); }} onApply={() => { setApplied({ ...f }); setPage(1); }} filters={[{ name: "entity", label: "Entidade", type: "text" }, { name: "entity_id", label: "ID do registro", type: "text" }, { name: "user_id", label: "Usuário", type: "ref", resource: "users" }, { name: "created_at_from", label: "De", type: "date" }, { name: "created_at_to", label: "Até", type: "date" }]} />
    <DataTable rows={q.data?.items ?? []} total={q.data?.total} page={page} pageSize={pageSize} onPage={setPage} onPageSize={setPageSize} loading={q.isLoading} onRowClick={setRow} columns={[{ key: "created_at", label: "Data/hora", render: (r) => dateTimeBR(r["created_at"] as string) }, { key: "user_name", label: "Usuário" }, { key: "entity", label: "Entidade" }, { key: "action", label: "Ação", render: (r) => <Badge tone={r["action"] === "delete" || r["action"] === "cancel" ? "red" : r["action"] === "create" ? "green" : "slate"}>{String(r["action"])}</Badge> }, { key: "entity_id", label: "Registro", className: "font-mono text-[11px]" }, { key: "ip", label: "IP" }]} />
    <Dialog open={Boolean(row)} onOpenChange={() => setRow(null)} title="Detalhe do evento" size="lg">{row && <div className="grid gap-2 text-xs md:grid-cols-2"><div><h4 className="font-semibold">Antes</h4><pre className="max-h-80 overflow-auto rounded bg-slate-50 p-2">{JSON.stringify(row["before"], null, 2) ?? "—"}</pre></div><div><h4 className="font-semibold">Depois / metadados</h4><pre className="max-h-80 overflow-auto rounded bg-slate-50 p-2">{JSON.stringify(row["after"] ?? row["metadata"], null, 2) ?? "—"}</pre></div></div>}</Dialog>
  </CardBody></Card>;
}
