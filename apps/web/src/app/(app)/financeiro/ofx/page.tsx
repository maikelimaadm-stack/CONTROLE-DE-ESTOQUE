...
export default function Page() {
  ...
  return <Card><CardHeader title="Conciliação Bancária (OFX)" subtitle="Importe o extrato OFX e concilie cada transação com um movimento existente ou crie um novo." actions={can("ofx_imports.create") && <Button size="sm" onClick={() => setOpen(true)}>Importar OFX</Button>} /><CardBody>
    {q.isLoading ? <Spinner /> : <DataTable rows={q.data?.items ?? []} onRowClick={(r) => router.push(`/financeiro/ofx/${r["id"]}`)} columns={[{ key: "code", label: "Cód." }, { key: "description", label: "Descrição" }, { key: "bank_account_name", label: "Conta" }, { key: "start_date", label: "De", render: (r) => dateBR(r["start_date"] as string) }, { key: "end_date", label: "Até", render: (r) => dateBR(r["end_date"] as string) }, { key: "transaction_count", label: "Transações", align: "right" }, { key: "status", label: "Status", render: (r) => <Badge tone={r["status"] === "reconciled" ? "green" : "amber"}>{String(r["status"])}</Badge> }, { key: "created_at", label: "Importado em", render: (r) => dateTimeBR(r["created_at"] as string) }]} />}
