...
import { brl, dateBR, dateTimeBR, pct } from "@/lib/utils";
...
export default function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <LoadingOr q={q}>{d && <DetailShell title={`Movimento bancário ${String(d["code"] ?? "")}`} back="/financeiro?tab=caixa&sub=extrato" status={String(d["status"])} actions={d["status"] === "confirmed" && can("bank_movements.delete") && <Button size="sm" variant="danger" onClick={() => setC(true)}>Cancelar movimento</Button>}>
    <KV items={[["Conta", `${d["bank_account_name"]} (${d["agency"] ?? ""}/${d["account_number"] ?? ""})`], ["Data", dateBR(d["movement_date"] as string)], ["Tipo", d["type"] === "in" ? "Entrada" : "Saída"], ["Categoria", enumLabel("bank_category_type", d["category_type"])], ["Valor", brl(d["amount"] as string)], ["Juros", brl(d["interest"] as string)], ["Documento", String(d["document"] ?? "—")], ["Empresa", String(d["empresa_name"] ?? "—")], ["Pessoa", String(d["person_name"] ?? "—")], ["Proprietário", String(d["proprietary_name"] ?? "—")], ["Conta destino", String(d["destination_account_name"] ?? "—")], ["Origem", enumLabel("source_type", d["source_type"] ?? "manual")], ["Conciliado", d["reconciled_at"] ? dateTimeBR(d["reconciled_at"] as string) : "Não"], ["Observação", String(d["note"] ?? "")]]} />
    <h3 className="text-xs font-semibold uppercase text-brand-700">Rateio</h3>
    <SimpleTable rows={d.apportionments} cols={[{ key: "category_code", label: "Código" }, { key: "category_name", label: "Natureza" }, { key: "cost_center_name", label: "Centro de resultado" }, { key: "chart_account_name", label: "Conta contábil" }, { key: "percentage", label: "%", align: "right", render: (r) => pct(r["percentage"] as string, 2) }, { key: "amount", label: "Valor", align: "right", render: (r) => brl(r["amount"] as string) }]} />
    {d.settlements.length > 0 && <><h3 className="text-xs font-semibold uppercase text-brand-700">Baixas de títulos vinculadas</h3><SimpleTable rows={d.settlements} cols={[{ key: "code", label: "Código" }, { key: "number", label: "Título", render: (r) => <Link className="text-brand-700 underline" href={`/financeiro/contas-a-pagar/${r["title_id"]}`}>{String(r["number"])}</Link> }, { key: "net_amount", label: "Líquido", align: "right", render: (r) => brl(r["net_amount"] as string) }]} /><p className="text-xs text-amber-700">Movimentos gerados por baixa de título só podem ser cancelados pela tela do título (cancelar baixa).</p></>}
