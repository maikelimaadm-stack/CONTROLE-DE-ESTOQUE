...
export default function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <LoadingOr q={q}>{d && <DetailShell title={`Movimento bancário ${String(d["code"] ?? "")}`} back="/financeiro/movimentos" status={String(d["status"])} actions={d["status"] === "confirmed" && can("bank_movements.delete") && <Button size="sm" variant="danger" onClick={() => setC(true)}>Cancelar movimento</Button>}>
