...
test("CADASTROS AJUSTES 01 · AJ-K3 — ficha de Parceiro da web NOVA salva contra a base: sem a 0030 os campos novos não aparecem nem viajam (200); com a 0030 aparecem, viajam e voltam pelo banco", async ({ page }) => {
  await login(page);
  const cab = await cabecalhosDaSessao(page);
  ...
  expect(r.status(), await r.text()).toBe(201);
  const id = (await r.json() as { id: string }).id;
  expect([matriz, id], "os ids vêm da API e entram no SQL — têm de ser UUID").toEqual([expect.stringMatching(UUID), expect.stringMatching(UUID)]);
  ...
  if (!tem0030) {
    ...
    for (const [nome, vizinho, campos] of porAba) {
      ...
      await expect(page.getByLabel(vizinho, { exact: true }), `premissa: a aba ${nome} abriu`).toBeVisible();
    ...
    for (const [nome, vizinho, campos] of porAba) {
      await aba(nome).click();
      await expect(page.getByLabel(vizinho, { exact: true }), `premissa: a aba ${nome} abriu`).toBeVisible();
      for (const rotulo of campos) await expect(page.getByLabel(rotulo, { exact: true }), `${rotulo} aparece contra a base com a 0030`).toBeVisible();
    }
    await aba("Identificação").click();
    ...
    await page.getByLabel("Latitude", { exact: true }).fill(valores.latitude);
    await page.getByLabel("Longitude", { exact: true }).fill(valores.longitude);
    ...
    await expect(page.getByLabel("Latitude", { exact: true })).toHaveValue(/^-15[.,]228/);
    await expect(page.getByLabel("Longitude", { exact: true })).toHaveValue(/^-59[.,]335/);
    await aba("Contatos").click();
