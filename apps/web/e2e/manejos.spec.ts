...
async function fixtures(page: Page) {
  const animals = await apiCall<{ items: { id: string; empresa_id: string; batch_id: string | null }[] }>(page, "GET", "/api/livestock/animals?pageSize=1");
  ...
  return { animalId: a.id, empresaId: a.empresa_id, batchId: a.batch_id ?? batches.items[0]?.id ?? null };
}
...
test.describe("manejos: detalhe real (sem 404), breadcrumb, retorno com filtro e rota direta", () => {
  test("manejo sanitário: lista → filtro de tipo → Visualizar → registro correto → Voltar mantém o filtro", async ({ page }) => {
    ...
    const h = await apiCall<{ id: string; code: string }>(page, "POST", "/api/livestock/handlings", { empresa_id: fx.empresaId, handling_type: "sanitary", handling_date: "2026-09-10", batch_id: fx.batchId, dose: "2", note: "e2e manejo sanitário", items: [{ animal_id: fx.animalId, quantity: "1" }] });
  ...
  test("pesagem: chip Pesagens → Visualizar abre /pecuaria/pesagens/:id com GMD; rota genérica /manejo/weighing/:id redireciona", async ({ page }) => {
    await login(page); const fx = await fixtures(page);
    const w = await apiCall<{ id: string; code: string }>(page, "POST", "/api/livestock/weighings", { empresa_id: fx.empresaId, weighing_date: "2026-09-10", batch_id: fx.batchId, items: [{ animal_id: fx.animalId, weight: "301" }] });
  ...
  test("rota direta: registro inexistente mostra erro (sem 404 de página) e usuário sem permissão recebe 403", async ({ page }) => {
    ...
    const h = await apiCall<{ id: string }>(page, "POST", "/api/livestock/handlings", { empresa_id: fx.empresaId, handling_type: "nutrition", handling_date: "2026-09-10", batch_id: fx.batchId, items: [{ animal_id: fx.animalId, quantity: "1" }] });
