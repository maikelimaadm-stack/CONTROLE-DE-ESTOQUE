...
export const EXCECOES_ESCOPO: Readonly<Record<string, string>> = {
  ...
  "report.birthdays": "Aniversariantes e quadro de pessoal saem de `erp.people` + `erp.employee_profiles`, cadastros da ORGANIZAÇÃO. Desde a 0028 a ficha tem `empresa_id` = LOTAÇÃO informativa e anulável (ver `employee_profiles` abaixo), não empresa de lançamento: o quadro continua da organização. Os LANÇAMENTOS de pessoal (apuração, adiantamentos) continuam de empresa, no módulo pessoas_rh.",
  "report.active_employees": "Mesmo caso de report.birthdays: quadro de pessoal ativo é cadastro da organização (erp.people/erp.employee_profiles); a lotação (0028) é informativa e anulável.",
  employee_profiles: "Ficha de RH 1:1 do PARCEIRO (cadastro da organização, recurso `funcionarios`/`employees`). A coluna `empresa_id` (0028) é a LOTAÇÃO informativa e anulável: recortar a ficha por ela esconderia o funcionário sem lotação a quem administra o RH da organização. A empresa gravada é conferida contra o escopo de lançamento de quem grava e pela FK composta com a organização.",
