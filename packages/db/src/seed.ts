...
export async function seedDemo(db: Db, opts: { orgName?: string; adminEmail?: string; adminPassword?: string; slug?: string } = {}, log: (m: string) => void = console.log): Promise<DemoOrg> {
  ...
  return withTx(db, { orgId: null, userId: null }, async (tx) => {
    ...
    await tx.query("insert into erp.role_permissions(role_id,permission_key) select $1,key from erp.permissions where key in ('products.view','products.create','products.edit','products.export') or key like 'stocks.%' or key like 'requisitions.%' or key like 'input_entries.%' or key like 'purchase_requests.view' or key like 'purchase_requests.create' or key like 'dashboard.home.%' or key like 'warehouses.view' on conflict do nothing", [opRole.rows[0]!.id]);
